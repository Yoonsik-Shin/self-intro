...
export function RelatedExperienceLinks({ experienceId, workspaceSlug, onNavigate }: Props) {
    ...
    return (
        <div className="mt-2 border-t border-slate-100 pt-2.5 print:hidden">
            <p className="resume-label mb-2 font-bold text-slate-600">관련 프로젝트 · 이력</p>
            <div className="flex flex-wrap gap-1.5">
                {data.map((experience) => (
                    <button
                        ...
                        className="resume-meta rounded-md border border-violet-100 bg-violet-50/50 px-2.5 py-1.5 font-semibold text-violet-700 transition hover:border-violet-200 hover:bg-violet-50"
                    >
                        {experience.title}
                    </button>
                ))}
