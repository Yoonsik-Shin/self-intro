...
const DISPLAY_CATEGORIES = ['Backend & Language', 'Frontend', 'Database', 'DevOps & Infra', 'AI / RAG', 'Others'];
...
export function SkillsSection({ skills, experiences, milestones, onSelectMilestone }: Props) {
  ...
  const selectedSkill = useMemo(() => groups.flatMap((g) => g.skills).find((s) => s.id === selectedSkillId), [groups, selectedSkillId]);

  const selectedSkillExperiences = useMemo(() => {
    if (!selectedSkill) return [];
    return experiences
      .filter((experience) => experience.skills.some((skill) => skill.id === selectedSkill.id))
          ...
          {groups.map((group) => (
            <div key={group.value} className="space-y-4">
              ...
              {group.skills.length === 0 ? (
                <p className="border-l-4 border-slate-200 px-3 py-1.5 text-sm font-bold text-slate-400">선택된 기술이 없습니다.</p>
              ) : (
                              ...
                              {skill.skillVersion && (
                                <span
                                  className={`shrink-0 rounded-md border px-1 py-0.5 text-[10px] font-black leading-none ${
                                    selectedSkillId === skill.id ? 'border-white/25 bg-white/15 text-white' : 'border-slate-200 bg-slate-50 text-slate-500'
                                  }`}
                                >
                ...
                {selectedSkillExperiences.length > 0 ? (
                  selectedSkillExperiences.map((experience) => (
                    <button
                      ...
                      key={experience.id}
                      onClick={() => {
                        const milestone = milestones.find((item) => item.title === experience.title);
                  ...
                  <p className="px-1 py-3 text-sm font-bold text-slate-400">연결된 experience가 없습니다.</p>
                )}
              </div>
              {selectedSkill.id > 0 && <RelatedStudyNotes skillId={selectedSkill.id} />}
            </div>
          )}
        </div>
      </div>
