...
export function ProjectsSection({ milestones, selectedMilestoneId, onSelectMilestone, expandedDetailIds, onToggleDetail, onSetExpandedDetailIds, onNavigateRelatedExperience }: Props) {
  ...
  const toggleAllDetails = (details: ExperienceDetail[]) => {
    ...
    onSetExpandedDetailIds(shouldCollapse ? expandedDetailIds.filter((id) => !ids.includes(id)) : [...new Set([...expandedDetailIds, ...ids])]);
  };
  ...
  return (
    <section id="projects" className="scroll-mt-24 space-y-6">
      <div className={cardStyle}>
        ...
        <div className={`resume-project-list relative mt-8 space-y-8 ${milestones.length > 0 ? 'before:absolute before:bottom-4 before:left-[15px] before:top-4 before:w-[2px] before:bg-slate-200' : ''}`}>
          {milestones.length === 0 && (
          ...
          {milestones.map((m) => (
            ...
            >
              <div className={`resume-project-bullet absolute left-[7px] top-1.5 z-10 h-[18px] w-[18px] rounded-full border-4 border-white shadow-sm transition-colors ${selectedMilestoneId === m.id ? 'scale-110 bg-slate-900' : 'bg-slate-300 group-hover:bg-slate-500'}`} />

              <div className={`resume-project-card space-y-4 rounded-xl border p-6 shadow-sm transition-all duration-300 ${selectedMilestoneId === m.id ? 'border-slate-800 bg-white ring-2 ring-slate-100/50' : 'border-slate-200/80 bg-slate-50/50 hover:border-slate-400 hover:bg-white'}`}>
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-3">
                  <div>
                    ...
                    <h3 className="resume-item-title mt-1.5 font-black text-slate-800">{m.title}</h3>
                  </div>
                  {m.repositoryUrl && (
                    <a
                ...
                <div className="space-y-4">
                  ...
                  {m.tags.length > 0 && (
                    <div>
                      <h4 className="resume-label mb-1.5 font-bold uppercase tracking-wider text-slate-400">태그</h4>
                          ...
                          return (
                            <li key={detail.id} id={`experience-detail-${detail.id}`} className="scroll-mt-24 py-1.5 first:pt-0.5 last:pb-0.5">
                              ...
                              >
                                ...
                                <span className={`resume-body min-w-0 text-sm leading-5 transition sm:text-[0.9375rem] ${hasDetailContent ? 'font-medium text-slate-700 group-hover:text-slate-900' : 'text-slate-500'}`}>{detail.content}</span>
                                {detail.id > 0 && (
                                  <Link
