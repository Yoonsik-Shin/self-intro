...
export function CompetenciesSection({ competencies, milestones, onSelectMilestone }: Props) {
  ...
  const expandableIds = useMemo(() => ordered.filter((c) => c.evidences.length > 0 || c.relatedStudies.length > 0).map((c) => c.id), [ordered]);
  const isAllExpanded = expandableIds.length > 0 && expandableIds.every((id) => expandedIds.includes(id));

  const toggle = (id: number) => setExpandedIds((current) => (current.includes(id) ? current.filter((x) => x !== id) : [...current, id]));
                    ...
                    {expandedIds.includes(competency.id) && (
                      <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (evidence.experienceType === 'PROJECT') {
                                const milestone = milestones.find((item) => item.experienceId === evidence.experienceId);
                                if (milestone) onSelectMilestone(milestone.id);
                                scrollToElement(`project-experience-${evidence.experienceId}`);
                              } else {
                        ...
                        {competency.relatedStudies.map((study) => (
                          <Link key={`study-${study.id}`} href={`/study/${encodeURIComponent(study.slug)}`} className="font-bold text-blue-700 hover:underline">
                            {study.title}
                          </Link>
                        ))}
                      </div>
