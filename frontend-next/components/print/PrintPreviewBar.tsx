    ...
    return (
        <div className="relative z-50 flex h-14 w-full shrink-0 items-center justify-between gap-4 bg-slate-900 px-4 shadow-xl print:hidden sm:px-6 border-b border-slate-800">
            ...
            <div className="flex items-center gap-2 shrink-0">
                {(onUndo || onRedo) && (
                    <div className="flex items-center rounded-xl border border-slate-700/60 bg-slate-800 p-1">
                        <button
                            type="button"
                            onClick={onUndo}
                            disabled={!canUndo}
                            className="grid h-7 w-7 place-items-center rounded-lg text-slate-300 transition hover:bg-slate-700 hover:text-white disabled:cursor-not-allowed disabled:text-slate-600 disabled:hover:bg-transparent"
                            title="실행 취소 (Ctrl/⌘ + Z)"
                            aria-label="실행 취소"
                        >
                ...
                {onToggleMarginSettings && (
                    <button
                        ...
                        className={`flex h-9 items-center gap-1.5 rounded-xl border px-2.5 text-[10px] font-black transition ${
                            marginSettingsOpen
                ...
                {integratedDocumentSettings && (onSaveServer || onSaveLocal) && (
                    <div ref={saveMenuRef} className="relative">
                        <button
                            ...
                            className="flex items-center gap-1.5 rounded-lg border border-blue-500/50 bg-blue-500/20 px-3 py-1.5 text-xs font-bold text-blue-100 transition hover:bg-blue-500/30 hover:text-white"
                            title="현재 인쇄 설정 저장"
                        >
                            <Save className="h-3.5 w-3.5" />
                        ...
                        {saveMenuOpen && (
                            <div className="absolute right-0 top-full z-[80] mt-2 w-56 overflow-hidden rounded-xl border border-slate-700 bg-slate-950 p-1.5 text-left shadow-2xl">
                                {onSaveServer && (
                                    <button
                                        type="button"
                                        onClick={() => {
                                        ...
                                        className="flex w-full items-start gap-2 rounded-lg px-3 py-2 text-slate-200 transition hover:bg-slate-800 hover:text-white"
                                    >
