...
export function PrintPreviewBar({
    ...
    const zoomControl = (
        <div className="flex h-9 items-center gap-1 bg-slate-800 border border-slate-700/60 rounded-md px-1.5 text-white shrink-0">
            <button
    ...
    const lineHeightControl =
        lineHeight !== undefined && onLineHeightChange ? (
            <div
                className="flex h-9 items-center gap-1.5 bg-slate-800 border border-slate-700/60 rounded-md px-2 text-white shrink-0"
                title="본문 줄간격 조절"
            >
    ...
    return (
        <div className="relative z-50 flex h-14 w-full shrink-0 items-center justify-between gap-4 bg-slate-900 px-4 shadow-xl print:hidden sm:px-6 border-b border-slate-800">
            ...
            <div className="flex items-center gap-2 shrink-0">
                {(onUndo || onRedo) && (
                    <div className="flex h-9 items-center rounded-md border border-slate-700/60 bg-slate-800 p-1">
                        <button
                            type="button"
                            onClick={onUndo}
                            disabled={!canUndo}
                ...
                {!integratedDocumentSettings && (
                    ...
                    >
                        ...
                        <span className="hidden xl:inline">구성 관리</span>
                    </button>
                )}
                {onToggleAiChat && (
                    <button
                ...
                {!integratedDocumentSettings && onOpenTemplateModal && (
                    <button
                        ...
                        className="flex h-9 items-center gap-1.5 rounded-md border border-slate-700 bg-slate-800 px-3 text-xs font-bold text-slate-200 transition hover:border-slate-500 hover:text-white"
                        title="다른 저장된 템플릿 불러오기 또는 변경"
                    >
                        <FolderOpen className="h-3.5 w-3.5" />
                        <span className="hidden xl:inline">템플릿 불러오기</span>
                    </button>
                )}
                {!integratedDocumentSettings && onSaveServer && (
                    <button
                        onClick={onSaveServer}
                        className="flex h-9 items-center gap-1.5 rounded-md border border-blue-500/50 bg-blue-500/20 px-3 text-xs font-bold text-blue-200 transition hover:bg-blue-500/30 hover:text-white"
                        title="현재 인쇄 설정을 서버 템플릿으로 저장"
                    >
                        <Save className="h-3.5 w-3.5" />
                        <span className="hidden xl:inline">템플릿으로 저장</span>
                    </button>
                )}
                {!integratedDocumentSettings && onSaveLocal && (
                    <button
                        onClick={onSaveLocal}
                        className="flex h-9 items-center gap-1.5 rounded-md border border-amber-500/50 bg-amber-500/10 px-3 text-xs font-bold text-amber-300 transition hover:bg-amber-500/20 hover:text-white"
                        title="현재 설정을 내 브라우저에 저장"
                    >
                        <Save className="h-3.5 w-3.5" />
                        <span className="hidden xl:inline">내 로컬 저장</span>
                    </button>
                )}
                {integratedDocumentSettings && (onSaveServer || onSaveLocal) && (
                    <div ref={saveMenuRef} className="relative">
                        <button
                            ...
                            className="flex h-9 items-center gap-1.5 rounded-md border border-blue-500/50 bg-blue-500/20 px-3 text-xs font-bold text-blue-100 transition hover:bg-blue-500/30 hover:text-white"
                            title="현재 인쇄 설정 저장"
                        >
                            <Save className="h-3.5 w-3.5" />
                ...
                <button
                    onClick={onPrint}
                    className="flex h-9 items-center gap-1.5 rounded-md bg-white px-3 text-xs font-black text-slate-900 shadow-sm transition hover:bg-slate-100"
                    title="인쇄"
                >
                    <Printer className="h-3.5 w-3.5" />
                    <span className="hidden xl:inline">인쇄</span>
                </button>
                <button
                    onClick={onCancel}
