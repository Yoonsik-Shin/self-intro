    ...
    <div className="relative z-50 flex h-14 w-full shrink-0 items-center justify-between gap-4 bg-slate-900 px-4 shadow-xl print:hidden sm:px-6 border-b border-slate-800">
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:gap-2 min-w-0">
          ...
          {excludedCount > 0 && <span className="text-xs font-semibold text-red-400 whitespace-nowrap">({excludedCount}개 제외)</span>}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        {onToggleHideGuides && (
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-bold transition ${
              hideGuides ? 'border-indigo-400/80 bg-indigo-600 text-white shadow-sm' : 'border-slate-700 bg-slate-800 text-slate-300 hover:border-slate-500 hover:text-white'
            }`}
        ...
        <button
          ...
          className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-bold transition ${
            navOpen ? 'border-slate-400 bg-slate-700 text-white' : 'border-slate-700 bg-slate-800 text-slate-300 hover:border-slate-500 hover:text-white'
          }`}
        >
        ...
        <button onClick={onPrint} className="flex items-center gap-1.5 rounded-lg bg-white px-3.5 py-1.5 text-xs font-black text-slate-900 shadow-sm transition hover:bg-slate-100">
          <Printer className="h-3.5 w-3.5" />
          인쇄
        </button>
        <button onClick={onCancel} className="grid h-7 w-7 place-items-center rounded-lg text-slate-400 transition hover:bg-slate-800 hover:text-white" aria-label="프리뷰 닫기">
          <X className="h-4 w-4" />
        </button>
      </div>
    </div>
