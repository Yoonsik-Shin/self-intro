...
import { Printer, Settings, FileText, Trash2, X, Save, ChevronRight, ArrowLeft, Pencil } from 'lucide-react';
import { printTemplateApi } from '@/lib/api';
import type { PrintTemplate } from '@/lib/api/types';
import { getLocalSaves, removeLocal, renameLocal, type LocalPrintSave } from '@/lib/printTemplateLocal';
...
export function PrintModeModal({ open, onClose, onManual, onApplyTemplate }: PrintModeModalProps) {
  ...
  const handleSelectServer = (t: PrintTemplate) => {
    ...
    const { __forcedPageOverrides, ...pureGaps } = rawGaps as Record<string, number> & { __forcedPageOverrides?: unknown };
    onApplyTemplate({
      excludedIds: t.excludedIds || [],
      sectionOrder: t.sectionOrder || [],
      sectionGaps: pureGaps || {},
      forcedPageOverrides: __forcedPageOverrides && typeof __forcedPageOverrides === 'object' ? (__forcedPageOverrides as Record<string, number>) : {},
    });
  };
      ...
      <div className="relative w-full max-w-md rounded-3xl bg-white shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-100 px-6 pt-6 pb-4.5">
          <div className="flex items-center gap-3">
            {step === 'TEMPLATE_LIST' ? (
              <button onClick={() => setStep('MAIN')} className="grid h-8.5 w-8.5 place-items-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-slate-900" title="이전 단계로">
            ...
            <h2 className="text-lg font-black text-slate-900">{step === 'MAIN' ? 'PDF 인쇄' : '템플릿 선택'}</h2>
          </div>
          <button onClick={onClose} className="grid h-8.5 w-8.5 place-items-center rounded-xl text-slate-400 transition hover:bg-slate-100 hover:text-slate-600" aria-label="닫기">
          ...
          <div className="px-6 pt-5 pb-4 space-y-3.5">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <span className="block text-base font-black text-slate-900">직접 조정하기</span>
                  <span className="mt-0.5 block text-xs font-semibold text-slate-500">섹션 순서, 포함/제외를 직접 설정합니다</span>
                </div>
              </div>
            </button>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="block text-base font-black text-slate-900 group-hover:text-blue-700">저장된 템플릿 선택하기</span>
                    {totalTemplateCount > 0 && <span className="rounded-full bg-blue-100 px-2 py-0.5 text-xs font-black text-blue-700">{totalTemplateCount}</span>}
                  </div>
                  <span className="mt-0.5 block text-xs font-semibold text-slate-500">{totalTemplateCount > 0 ? '미리 준비된 설정으로 빠르게 인쇄합니다' : '저장된 인쇄 템플릿이 있습니다'}</span>
                </div>
              </div>
        ...
        {step === 'TEMPLATE_LIST' && (
          <div className="max-h-[55vh] overflow-y-auto px-6 pt-5 pb-4 space-y-5">
            {totalTemplateCount === 0 ? (
              <div className="py-8 text-center text-sm font-semibold text-slate-400">저장된 템플릿이나 로컬 설정이 없습니다.</div>
            ) : (
              <>
                {serverTemplates.length > 0 && (
                  <div>
                    <div className="mb-2.5 flex items-center gap-2">
                      <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">저장된 템플릿</span>
                      ...
                      {serverTemplates.map((t) => (
                        ...
                        >
                          <div className="flex items-center gap-3.5 min-w-0">
                            ...
                            <span className="truncate text-sm font-bold text-slate-800 group-hover:text-blue-700">{t.name}</span>
                          </div>
                ...
                {localSaves.length > 0 && (
                  <div>
                    <div className="mb-2.5 flex items-center gap-2">
                      <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">내 브라우저 저장 설정</span>
                      <div className="h-px flex-1 bg-slate-200" />
                    </div>
                    <div className="space-y-2">
                      {localSaves.map((s) => (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              <span className="block truncate text-sm font-bold text-slate-800 group-hover:text-amber-800">{s.memo || '(설명 없음)'}</span>
                              <span className="text-[11px] font-semibold text-slate-400">
                                {formatDate(s.savedAt)} 저장
                                {s.excludedIds && s.excludedIds.length > 0 && ` · ${s.excludedIds.length}개 숨김`}
                              </span>
                            </div>
                          </div>
                          <div className="flex items-center gap-1 shrink-0">
                            <button onClick={(e) => handleRenameLocal(s, e)} className="grid h-8 w-8 place-items-center rounded-xl text-slate-300 transition hover:bg-amber-100 hover:text-amber-700" title="이름 변경">
                              <Pencil className="h-3.5 w-3.5" />
                            </button>
                            <button onClick={(e) => handleRemoveLocal(s.id, e)} className="grid h-8 w-8 place-items-center rounded-xl text-slate-300 transition hover:bg-red-50 hover:text-red-500" title="삭제">
        ...
        <div className="flex items-center gap-2.5 border-t border-slate-100 px-6 pt-3.5 pb-5">
          {step === 'TEMPLATE_LIST' && (
            <button onClick={() => setStep('MAIN')} className="flex-1 rounded-2xl border border-slate-200/90 py-2.5 text-sm font-bold text-slate-600 transition hover:bg-slate-50">
              이전
            </button>
          )}
          <button onClick={onClose} className="flex-1 rounded-2xl border border-slate-200/90 py-2.5 text-sm font-bold text-slate-600 transition hover:bg-slate-50">
            취소
          </button>
        </div>
      </div>
