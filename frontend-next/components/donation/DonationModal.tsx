...
type Phase = 'select' | 'creating' | 'waiting' | 'success' | 'canceled' | 'failed' | 'popupBlocked' | 'timeout';
...
export function DonationModal({ onClose }: DonationModalProps) {
  ...
  const submit = async () => {
    if (!amountValid) {
      setError(`후원 금액은 ${MIN_AMOUNT.toLocaleString()}원 이상 ${MAX_AMOUNT.toLocaleString()}원 이하로 입력해주세요.`);
      return;
    }
    ...
    try {
      ...
      popup?.close();
      setError(submitError instanceof ApiError ? submitError.message : '후원 요청에 실패했습니다. 잠시 후 다시 시도해주세요.');
      setPhase('select');
    }
  };
    ...
    >
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
        <div className="flex items-center justify-between">
          ...
          {closable && (
            <button onClick={onClose} className="rounded-lg p-1 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700" aria-label="닫기">
              <X className="h-5 w-5" />
            </button>
          )}
        </div>
        ...
        {phase === 'select' && (
          <div className="mt-4 flex flex-col gap-4">
            <p className="text-sm text-slate-500">작은 후원이 큰 힘이 됩니다. 결제는 페이앱 창에서 안전하게 진행됩니다.</p>
              ...
              {PRESET_AMOUNTS.map((preset) => (
                <button
                  ...
                  className={`rounded-xl border px-3 py-2 text-sm font-extrabold transition ${
                    selectedPreset === preset ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300'
                  }`}
                >
        ...
        {phase === 'success' && (
          <div className="mt-6 flex flex-col items-center gap-4 pb-2 text-center">
            ...
            <div className="text-sm text-slate-600">
              <p className="text-base font-black text-slate-900">후원해주셔서 진심으로 감사합니다! 💙</p>
              <p className="mt-1">보내주신 응원을 큰 힘 삼아 더 좋은 개발자가 되겠습니다.</p>
            </div>
            <button onClick={onClose} className="h-10 w-full rounded-xl bg-slate-900 text-sm font-extrabold text-white transition hover:bg-slate-700">
              닫기
            </button>
          </div>
        )}
        ...
        {phase === 'canceled' && (
          <div className="mt-6 flex flex-col items-center gap-4 pb-2 text-center text-sm text-slate-600">
            <AlertTriangle className="h-8 w-8 text-amber-500" />
            <p>결제가 취소되었습니다.</p>
            <button onClick={retry} className="h-10 w-full rounded-xl border border-slate-200 text-sm font-extrabold text-slate-600 transition hover:border-slate-300">
              다시 시도
            </button>
          </div>
        )}
