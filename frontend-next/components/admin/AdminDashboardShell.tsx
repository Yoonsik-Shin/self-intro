...
import { Home, LogOut, BookOpen, User, Cpu, Briefcase, ChevronLeft, ChevronRight, Eye, RefreshCw, Sparkles, Pin, Terminal, Printer, BarChart3, Heart, X } from 'lucide-react';
...
export function AdminDashboardShell() {
  ...
  const [pendingExperienceIntent, setPendingExperienceIntent] = useState<PendingExperienceIntent>(null);

  const { data: introData } = useQuery({ queryKey: ['introduction'], queryFn: bffApi.getIntroduction });
  ...
  const previewMaxAllowedWidth = Math.min(PREVIEW_MAX_WIDTH, Math.max(PREVIEW_MIN_WIDTH, viewportWidth - ADMIN_CONTENT_RESERVE_WIDTH));
  // 스택 브레이크포인트 아래에서는 미리보기가 뷰포트 전체를 차지한다(모바일 풀스크린).
  const effectivePreviewWidth = viewportWidth < PREVIEW_STACK_BREAKPOINT ? viewportWidth : Math.min(previewWidth, previewMaxAllowedWidth);
  ...
  const getPreviewTarget = (): { path: string; section?: string } => {
    switch (activeTab) {
      ...
      case 'EXPERIENCE': {
        ...
        const section = type === 'CAREER' ? 'career' : type === 'PROJECT' ? 'projects' : type === 'EDUCATION' || type === 'CERTIFICATE' ? 'credentials' : 'timeline';
        return { path: '/', section };
      }
      case 'ANALYTICS':
      case 'DONATIONS':
  ...
  const buildPreviewIntroData = (): IntroductionResponse | null => {
    ...
    if (activeTab === 'PROFILE' && profileDraft) {
      profile = { id: introData.profile?.id ?? 0, updatedAt: introData.profile?.updatedAt ?? new Date().toISOString(), ...profileDraft };
    }

    if (activeTab === 'SKILLS' && skillDraft) {
      const { studyIds: _studyIds, experienceIds: _experienceIds, experienceDetailIds: _experienceDetailIds, ...draftSkillFields } = skillDraft.form;
      const draftSkill: Skill = { id: skillDraft.editingId ?? -1, ...draftSkillFields };
      skills = skillDraft.editingId !== null ? skills.map((skill) => (skill.id === skillDraft.editingId ? draftSkill : skill)) : [...skills, draftSkill];
    }
    ...
    if (activeTab === 'EXPERIENCE' && experienceDraft) {
      ...
      const draftExperience: Experience = {
        ...
        contributionRate: form.type === 'PROJECT' && form.contributionRate != null ? Number(form.contributionRate) : undefined,
        repositoryUrl: form.type === 'PROJECT' ? form.repositoryUrl?.trim() || undefined : undefined,
      ...
      experiences = experienceDraft.editingId !== null ? experiences.map((experience) => (experience.id === experienceDraft.editingId ? draftExperience : experience)) : [...experiences, draftExperience];
    }
  ...
  }, [isPreviewOpen, activeTab, introData, profileDraft, skillDraft, experienceDraft, skillsList]);
  ...
  return (
    <main className="min-h-screen bg-[#f8fafc] text-slate-800">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm font-bold transition disabled:cursor-not-allowed disabled:opacity-40 ${
              isPreviewOpen ? 'border-slate-900 bg-slate-900 text-white hover:bg-slate-800' : 'border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-800'
            }`}
          >
            <Eye className="h-3.5 w-3.5" />
            미리보기
          </button>
          <a href="/" className="flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 text-sm font-bold text-slate-500 transition hover:bg-slate-50 hover:text-slate-800">
      ...
      <div
        ...
        style={{ gridTemplateColumns: isSidebarCollapsed ? '64px minmax(0, 1fr)' : '240px minmax(0, 1fr)' }}
      >
        <aside className={`relative min-w-0 lg:sticky lg:top-20 lg:self-start ${isSidebarCollapsed ? 'rounded-2xl border border-slate-200 bg-white px-2 py-3 shadow-sm' : ''}`}>
          <button
            type="button"
            ...
            className={`z-20 flex items-center justify-center text-slate-400 transition-colors hover:text-slate-900 ${
              isSidebarCollapsed ? 'relative mx-auto mb-3 h-8 w-8 shrink-0' : 'absolute -right-4 top-1 !m-0 h-10 w-8'
            }`}
          >
          ...
          <nav aria-label="관리자 메뉴" className="space-y-5">
            {ADMIN_MENU_GROUPS.map((group, groupIndex) => (
              <section key={group.label} className={groupIndex > 0 && isSidebarCollapsed ? 'border-t border-slate-200 pt-3' : ''}>
                <h2 className={`mb-1.5 px-3 text-[11px] font-black tracking-[0.12em] text-slate-400 ${isSidebarCollapsed ? 'sr-only' : ''}`}>{group.label}</h2>
                    ...
                    return (
                      <button
                        ...
                        className={`flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-sm font-bold transition ${isSidebarCollapsed ? 'mx-auto h-11 w-11 justify-center gap-0 p-0' : ''} ${
                          isActive ? 'bg-slate-900 text-white shadow-sm shadow-slate-800/10' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
                        }`}
                      >
      ...
      {isPreviewOpen && (
        <div
          ...
          style={{ height: 'calc(100vh - 5rem)', width: isPreviewVisible ? effectivePreviewWidth : 0 }}
        >
          <div onMouseDown={handlePreviewResizeStart} className="absolute left-0 top-0 z-10 hidden h-full w-2.5 -translate-x-1/2 cursor-col-resize touch-none group sm:block" title="드래그하여 너비 조절">
          ...
          <div className="flex h-full w-full flex-col">
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
              ...
              <div className="flex shrink-0 items-center gap-2">
                <button type="button" onClick={refreshPreview} title="새로고침" aria-label="미리보기 새로고침" className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-500 transition hover:border-slate-300 hover:text-slate-900">
                  <RefreshCw className="h-3.5 w-3.5" />
                </button>
                <button type="button" onClick={closePreviewPanel} title="닫기" aria-label="미리보기 닫기" className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-500 transition hover:border-slate-300 hover:text-slate-900">
                  <X className="h-3.5 w-3.5" />
                </button>
              </div>
            </div>
            <iframe key={`${previewNonce}-${getPreviewTarget().path}`} src={`${getPreviewTarget().path}?preview=1`} title="메인페이지 미리보기" className="w-full flex-1 border-0" />
          </div>
        </div>
      )}
      </div>
