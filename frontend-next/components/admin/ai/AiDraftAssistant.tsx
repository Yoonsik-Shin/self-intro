...
export function useAiSuggestionStream() {
  ...
  const pushStage = (stage: number, message: string) => {
    setAiStages((current) => [...current.map((item) => ({ ...item, done: true })), { stage, message, buffer: '', done: false }]);
  };

  const appendToken = (stage: number, text: string) => {
    setAiStages((current) => current.map((item) => (item.stage === stage ? { ...item, buffer: item.buffer + text } : item)));
  };

  const finishStages = () => setAiStages((current) => current.map((item) => ({ ...item, done: true })));
...
export function AiStageBubble({ stage, fieldLabels, extra }: { stage: AiStageProgress; fieldLabels: Record<string, string>; extra?: ReactNode }) {
  const lines = extractAiReadableLines(stage.buffer, fieldLabels);
  return (
    <div className="flex items-start gap-2.5">
      <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-violet-100 text-violet-600">{stage.done ? <Check className="h-3.5 w-3.5" /> : <Loader2 className="h-3.5 w-3.5 animate-spin" />}</span>
...
export function extractAiReadableLines(buffer: string, fieldLabels: Record<string, string>): AiReadableLine[] {
  ...
  if (partial) lines.push({ label: fieldLabels[partial[1]], text: unescapeJsonText(partial[2]), done: false });
  return lines;
}
