...
import { ArrowLeft, ArrowRight, BriefcaseBusiness, Building2, CalendarDays, ChevronDown, ChevronUp, ExternalLink, Github, Pencil, Pin, PinOff, Search, Tags, Trash2, Wrench } from 'lucide-react';
...
export function ExperienceDetailPanel({ experience, allExperiences, parentExperience, onBack, onEdit, onDelete, onSelectExperience }: ExperienceDetailPanelProps) {
  ...
  const childProjects = experience.type === 'CAREER' ? (allExperiences ?? []).filter((item) => item.type === 'PROJECT' && item.careerId === experience.id) : [];
    ...
    <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm animate-fadeIn">
      <div className="border-b border-slate-200 bg-slate-50/70 px-5 py-4 sm:px-7">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <button type="button" onClick={onBack} className="inline-flex items-center gap-1.5 text-sm font-bold text-slate-600 transition hover:text-slate-950 group">
          ...
          <div className="flex items-center gap-2">
            <button type="button" onClick={() => onEdit(experience)} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-600 transition hover:border-slate-300 hover:text-slate-950">
              <Pencil className="h-3.5 w-3.5" />
              수정
            </button>
            <button type="button" onClick={() => onDelete(experience.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-red-100 bg-white px-3 py-2 text-sm font-bold text-red-500 transition hover:border-red-200 hover:bg-red-50">
      ...
      <div className="grid gap-8 px-5 py-6 sm:px-7 lg:grid-cols-[minmax(0,1fr)_260px] lg:py-8">
        <div className="min-w-0 space-y-8">
            ...
            <div className="flex flex-wrap gap-3">
              {[...experience.images]
                ...
                .map((image) => (
                  ...
                  <img key={image.objectKey} src={image.url} alt="" className="h-28 w-28 rounded-xl border border-slate-200 object-cover" />
                ))}
            </div>
          )}
          ...
          {experience.type === 'CAREER' && childProjects.length > 0 && (
            <section>
              <div className="mb-4 flex items-center gap-2 border-b border-slate-100 pb-3">
                ...
                <h4 className="text-sm font-black uppercase tracking-wider text-slate-700">소속 직장 프로젝트 · {childProjects.length}개</h4>
              </div>
              ...
              <div className="space-y-3">
                {childProjects
                  ...
                  .filter(({ project }) => !projectSearch.trim() || project.title.toLowerCase().includes(projectSearch.trim().toLowerCase()) || (project.summary && project.summary.toLowerCase().includes(projectSearch.trim().toLowerCase())))
                  .map(({ project, index }) => {
                          ...
                          <div className="ml-9 mt-4 space-y-4 border-t border-slate-100 pt-3">
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-100">
                              {project.repositoryUrl ? (
                                <a href={project.repositoryUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-700 transition hover:text-slate-950 underline">
          ...
          {experience.details.length > 0 && (
            <section>
              <div className="mb-4 flex items-center gap-2 border-b border-slate-100 pb-3">
                <BriefcaseBusiness className="h-4 w-4 text-slate-500" />
                <h4 className="text-sm font-black uppercase tracking-wider text-slate-700">상세 경험 · {experience.details.length}개</h4>
              </div>
              ...
              <div className="space-y-3">
                {experience.details
                  ...
                  .filter(({ detail }) => !detailSearch.trim() || detail.content.toLowerCase().includes(detailSearch.trim().toLowerCase()))
                  .map(({ detail, index }) => {
                    ...
                    return (
                      <div key={detail.id} className="rounded-xl border border-slate-200 p-4">
                        <button type="button" onClick={() => hasContent && setExpandedDetailId(isExpanded ? null : detail.id)} className={`flex w-full items-start gap-3 text-left ${hasContent ? 'cursor-pointer' : 'cursor-default'}`}>
                          <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-900 text-xs font-black text-white">{index + 1}</span>
                          <span className="min-w-0 flex-1 font-black leading-snug text-slate-800">{detail.content}</span>
                          {hasContent && (isExpanded ? <ChevronUp className="mt-0.5 h-4 w-4 shrink-0 text-slate-400" /> : <ChevronDown className="mt-0.5 h-4 w-4 shrink-0 text-slate-400" />)}
                        </button>
                            ...
                            {detail.skills.length > 0 && (
                              <div className="mt-3 flex flex-wrap gap-1.5">
                                {detail.skills.map((skill) => (
                                  <span key={skill.id} className="rounded bg-slate-100 px-2 py-1 text-xs font-bold text-slate-600">
                                    {skill.name}
                                  </span>
                                ))}
                              </div>
                ...
                {experience.details.length > 0 && detailSearch.trim() && !experience.details.some((detail) => detail.content.toLowerCase().includes(detailSearch.trim().toLowerCase())) && (
                  <p className="rounded-xl border border-dashed border-slate-200 p-4 text-center text-xs font-semibold text-slate-400">검색 결과가 없습니다.</p>
                )}
              </div>
            </section>
        ...
        <aside className="space-y-5 lg:border-l lg:border-slate-100 lg:pl-6">
            ...
            <section className="rounded-xl border border-slate-200 p-3">
              ...
              <dl className="mt-2 space-y-2 text-xs">
                <div className="flex justify-between gap-3">
                  ...
                  <dd className="text-right font-bold text-slate-700">{experience.role ?? '-'}</dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-slate-400">기여도</dt>
                  <dd className="font-bold text-slate-700">{experience.contributionRate ?? 0}%</dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-slate-400">Slug</dt>
                  <dd className="truncate font-mono font-bold text-slate-700">{experience.slug ?? '-'}</dd>
                </div>
              </dl>
              {experience.repositoryUrl && (
                <a href={experience.repositoryUrl} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-lg bg-slate-900 px-3 py-2 text-xs font-bold text-white transition hover:bg-slate-800">
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-1.5">
                {experience.tags.map((tag) => (
                  <span key={tag.id} className="rounded-md bg-slate-100 px-2 py-1 text-xs font-bold text-slate-600">
                    #{tag.name}
                  </span>
                ))}
              </div>
          ...
          <section className="rounded-xl bg-slate-50 p-3 text-xs text-slate-500">
            <p>
              <span className="font-bold text-slate-700">정렬 순서</span> {experience.displayOrder}
            </p>
            {experience.timelineLabel && (
              <p className="mt-1">
                <span className="font-bold text-slate-700">타임라인 라벨</span> {experience.timelineLabel}
              </p>
            )}
          </section>
        </aside>
