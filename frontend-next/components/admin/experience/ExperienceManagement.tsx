...
import { ArrowDown, ArrowUp, BookOpen, Briefcase, Check, ChevronDown, ChevronUp, ListChecks, MinusCircle, Pencil, Pin, PinOff, Plus, PlusCircle, RefreshCw, Search, Trash2, WandSparkles, Wrench } from 'lucide-react';
import { ApiError, studyApi, skillApi, experienceApi, experiencePlacementApi, connectionApi } from '@/lib/api';
import type { Experience, ExperienceConnections, ExperienceDetailRequest, ExperienceDetailSuggestion, ExperienceRequest, ExperienceSuggestion, GalleryImage } from '@/lib/api/types';
...
export function ExperienceManagement({ pendingIntent, onConsumeIntent }: { pendingIntent: PendingExperienceIntent; onConsumeIntent: () => void }) {
  ...
  const selectedExperience = experiencesList?.find((experience) => experience.id === selectedExperienceId) ?? null;

  const filteredExperiences = experiencesList?.filter((exp) => {
    const matchesType = expFilter === 'ALL' || exp.type === expFilter;
    const matchesSearch = !expSearch || exp.title.toLowerCase().includes(expSearch.toLowerCase()) || (exp.summary && exp.summary.toLowerCase().includes(expSearch.toLowerCase()));
  ...
  const selectableExpSkills = (() => {
    ...
    return (skillsList ?? []).filter((skill) => skill.name.toLowerCase().includes(keyword) || skill.category.toLowerCase().includes(keyword));
  })();
  ...
  const selectableExpStudies = (studies ?? []).filter((study) => !expStudySearch || study.title.toLowerCase().includes(expStudySearch.toLowerCase()));

  const selectableRelatedExperiences = (experiencesList ?? []).filter(
    (experience) => experience.id !== expEditingId && (!expRelatedSearch || experience.title.toLowerCase().includes(expRelatedSearch.toLowerCase()) || experience.type.toLowerCase().includes(expRelatedSearch.toLowerCase())),
  );

  const buildExperienceConnections = (saved: Experience, form: AdminExperienceForm): ExperienceConnections => ({
    studyIds: form.studyIds,
    detailStudies: saved.details.map((detail, index) => ({
      detailId: detail.id,
      studyIds: form.details[index]?.studyIds ?? [],
    })),
    relatedExperiences: form.relatedExperienceIds.map((experienceId) => ({ experienceId, type: 'RELATED' as const })),
  });
  ...
  const createExpMutation = useMutation({
    mutationFn: async ({ payload, form, addToCoreProjects }: { payload: ExperienceRequest; form: AdminExperienceForm; addToCoreProjects: boolean }) => {
  ...
  const applyExpAiSummary = (suggestion: ExperienceSuggestion) => {
    if ((expForm.summary?.trim() || expForm.takeaway?.trim()) && !window.confirm('현재 작성한 요약·배운 점을 AI 초안으로 바꾸시겠습니까?')) return;
    setExpForm({
      ...expForm,
  ...
  const handleExpSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const payload: ExperienceRequest = {
      ...
      contributionRate: expForm.type === 'PROJECT' && expForm.contributionRate != null ? Number(expForm.contributionRate) : undefined,
  ...
  const openExperienceEditor = async (experience: Experience) => {
    try {
      ...
      const detailStudies = new Map(connections.detailStudies.map((connection) => [connection.detailId, connection.studyIds]));
      ...
      setExpForm({
        ...
        childProjectIds: (experiencesList ?? []).filter((item) => item.type === 'PROJECT' && item.careerId === experience.id).map((item) => item.id),
      });
      setExpAiSuggestions([]);
      resetExpAiStream();
      setIsExpFormOpen(true);
  ...
  const updateDetailField = (idx: number, field: 'content' | 'situation' | 'actionDetail' | 'outcome' | 'narrative', value: string) => {
    setExpForm({ ...expForm, details: expForm.details.map((d, i) => (i === idx ? { ...d, [field]: value } : d)) });
  };
  ...
  const moveDetailPoint = (idx: number, direction: -1 | 1) => {
    ...
    setExpandedDetailIdx((current) => (current === idx ? target : current === target ? idx : current));
  };
  ...
  const toggleDetailSkill = (idx: number, skillId: number) => {
    setExpForm({
      ...
      details: expForm.details.map((d, i) => {
        ...
        return { ...d, skillIds: isChecked ? d.skillIds.filter((id) => id !== skillId) : [...d.skillIds, skillId] };
      }),
    });
  };
  ...
  const toggleDetailStudy = (idx: number, studyId: number) => {
    setExpForm({
      ...
      details: expForm.details.map((detail, detailIndex) => {
        ...
        return { ...detail, studyIds: isChecked ? detail.studyIds.filter((id) => id !== studyId) : [...detail.studyIds, studyId] };
      }),
    });
  };
  ...
  const toggleExpSkill = (skillId: number) => {
    ...
    setExpForm({ ...expForm, skillIds: isChecked ? expForm.skillIds.filter((id) => id !== skillId) : [...expForm.skillIds, skillId] });
  };
          ...
          <div className="flex flex-wrap gap-1.5 w-full sm:w-auto">
            {['ALL', 'CAREER', 'PROJECT', 'EDUCATION', 'CERTIFICATE'].map((cat) => (
              <button
                ...
                className={`px-3 py-1.5 text-sm font-bold rounded-lg transition ${expFilter === cat ? 'bg-slate-900 text-white shadow-sm' : 'bg-slate-50 text-slate-500 hover:bg-slate-100 hover:text-slate-800 border border-slate-100'}`}
              >
                {cat === 'ALL' ? '전체' : cat === 'CAREER' ? '회사 경력' : cat === 'PROJECT' ? '프로젝트' : cat === 'EDUCATION' ? '학력' : '자격증'}
              </button>
            ))}
          </div>
          <div className="w-full sm:w-64">
            <input type="text" placeholder="이력명, 성과 검색..." value={expSearch} onChange={(e) => setExpSearch(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3.5 py-2 text-sm transition focus:border-slate-800 focus:outline-none bg-slate-50/50" />
          </div>
        </div>
      )}
      ...
      {isExpFormOpen && (
        <form onSubmit={handleExpSubmit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {createAsCoreProject && expEditingId === null && (
            <p className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-bold text-blue-700">프로젝트를 저장하면 핵심 프로젝트의 마지막 순서에 자동으로 편성됩니다.</p>
          )}
                  ...
                  <div ref={expAiChatRef} className="mt-4 max-h-80 space-y-2.5 overflow-y-auto rounded-xl border border-violet-100 bg-white p-3">
                    {expAiStages.map((stageItem) => (
                      <AiStageBubble
                        ...
                        extra={stageItem.stage === 1 && expAiFactCount > 0 ? <p className="mt-2 text-[11px] font-bold text-violet-600">검증된 사실 {expAiFactCount}개</p> : undefined}
                      />
                    ))}
                    {expAiError && <p className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">{expAiError}</p>}
                  </div>
                )}
                  ...
                  <div className="mt-4 space-y-3">
                    {expAiSuggestions.map((suggestion, index) => (
                      <article key={index} className="rounded-xl border border-violet-200 bg-white p-4 shadow-sm">
                        ...
                        {suggestion.takeaway && <p className="mt-2 text-xs leading-relaxed text-slate-500">배운 점: {suggestion.takeaway}</p>}
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 items-end rounded-xl border border-slate-200 bg-slate-50/50 p-4">
            <label className="flex items-center gap-2 text-sm font-bold text-slate-700">
              <input type="checkbox" checked={expForm.showOnTimeline} onChange={(e) => setExpForm({ ...expForm, showOnTimeline: e.target.checked })} className="h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-800" />
              커리어 & 학습 타임라인에 표시
            </label>
            <div className="sm:col-span-2">
          ...
          {expForm.type === 'CAREER' && (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-4 rounded-xl bg-slate-100/20 border border-slate-200/50 p-4">
              ...
              <div>
                ...
                <input type="text" required value={expForm.role} onChange={(e) => setExpForm({ ...expForm, role: e.target.value })} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-slate-800 focus:outline-none" />
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <ImageGalleryEditor scope="EXPERIENCE_GALLERY" images={expForm.images} onChange={(images) => setExpForm({ ...expForm, images })} />
          </div>
          ...
          <div>
            ...
            <MarkdownEditor value={expForm.summary ?? ''} onChange={(summary) => setExpForm({ ...expForm, summary })} />
          </div>

          <div>
            <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-400">Takeaway (성과 및 배운점, 마크다운)</label>
            <MarkdownEditor value={expForm.takeaway ?? ''} onChange={(takeaway) => setExpForm({ ...expForm, takeaway })} />
          </div>
          ...
          <div className="rounded-xl border border-slate-200 p-4 bg-slate-50/50">
            ...
            <div className="grid max-h-56 grid-cols-1 gap-2 overflow-auto sm:grid-cols-2 lg:grid-cols-3">
              {selectableExpSkills.map((s) => {
                ...
                return (
                  <label key={s.id} className={`flex items-start gap-2 p-2 rounded-lg border transition cursor-pointer text-sm ${isChecked ? 'bg-slate-100 border-slate-300 text-slate-950 font-bold' : 'bg-white border-slate-200 hover:border-slate-300'}`}>
                    <input type="checkbox" checked={isChecked} onChange={() => toggleExpSkill(s.id)} className="mt-0.5 h-3.5 w-3.5 rounded border-slate-300 text-slate-900 focus:ring-slate-800" />
              ...
              {selectableExpSkills.length === 0 && <p className="col-span-full py-4 text-center text-xs font-semibold text-slate-400">검색 결과가 없습니다.</p>}
            </div>
          </div>

          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-4">
              <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-slate-400">관련 Study · {expForm.studyIds.length}개</label>
              <div className="relative mb-2">
                <Search className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" />
                <input
                  type="search"
            ...
            {expForm.type === 'CAREER' && (
              <div className="rounded-xl border border-blue-200 bg-blue-50/40 p-4">
                <div className="mb-2">
                  <label className="block text-xs font-bold uppercase tracking-wider text-blue-900">소속 직장 프로젝트 (Child Projects) · {expForm.childProjectIds.length}개</label>
                  <p className="mt-0.5 text-xs text-blue-700 font-medium">이 직장 경력(CAREER) 기간 중 진행된 프로젝트 목록입니다. 선택 시 해당 프로젝트의 소속 경력(careerId)으로 연결됩니다.</p>
                </div>
                <div className="relative mb-2">
                  <Search className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" />
                  <input
                      ...
                      <label key={project.id} className="flex cursor-pointer items-start gap-2 rounded-lg border border-slate-200 bg-white p-2.5 text-xs hover:border-blue-300 transition">
                        <input
                          ...
                          onChange={() =>
                            setExpForm((current) => ({
                              ...current,
                              childProjectIds: current.childProjectIds.includes(project.id) ? current.childProjectIds.filter((id) => id !== project.id) : [...current.childProjectIds, project.id],
                            }))
                          }
                          className="mt-0.5"
                        />
            ...
            {expForm.type !== 'CAREER' && (
              <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-4">
                <div className="mb-2">
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-500">기타 연관 이력·참고 링크 (Cross References) · {expForm.relatedExperienceIds.length}개</label>
                  <p className="mt-0.5 text-xs text-slate-400 font-medium">이 항목과 연관된 다른 이력이나 경력을 연결합니다.</p>
                </div>
                <div className="relative mb-2">
                  <Search className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" />
                  <input
                    ...
                    <label key={experience.id} className="flex cursor-pointer items-start gap-2 rounded-lg border border-slate-200 bg-white p-2 text-xs">
                      <input
                        ...
                        onChange={() =>
                          setExpForm((current) => ({
                            ...current,
                            relatedExperienceIds: current.relatedExperienceIds.includes(experience.id) ? current.relatedExperienceIds.filter((id) => id !== experience.id) : [...current.relatedExperienceIds, experience.id],
                          }))
                        }
                        className="mt-0.5"
                      />
          ...
          {expForm.type !== 'CAREER' && (
            <>
              <div className="flex items-center gap-2 border-b border-slate-200 pb-2 pt-2">
                ...
                <h4 className="text-sm font-black uppercase tracking-wider text-slate-700">이력 상세 항목 (Bullet Points) · {expForm.details.length}개</h4>
              </div>
              ...
              <div className="rounded-xl border border-slate-200 p-4 bg-slate-50/50">
                <div className="flex gap-2 mb-3">
                  ...
                  />
                  <button type="button" onClick={addDetailPoint} className="rounded-lg bg-slate-100 border border-slate-300 text-slate-900 p-2 hover:bg-slate-200 text-xs font-bold flex items-center gap-1">
                ...
                <div className="space-y-2">
                  {expForm.details
                    .map((d, idx) => ({ d, idx }))
                    .filter(({ d }) => !detailListSearch.trim() || d.content.toLowerCase().includes(detailListSearch.trim().toLowerCase()))
                    .map(({ d, idx }) => {
                      ...
                      return (
                        <div key={idx} className="bg-white rounded-lg border border-slate-200 text-sm">
                          <div className="flex items-center justify-between gap-1 p-2">
                            ...
                            <button type="button" onClick={() => removeDetailPoint(idx)} className="text-red-500 transition hover:text-red-700">
                          ...
                          {isDetailExpanded && (
                            <div className="space-y-2 border-t border-slate-100 p-3">
                              <div>
                                ...
                                <MarkdownEditor value={d.situation ?? ''} onChange={(value) => updateDetailField(idx, 'situation', value)} />
                              </div>
                              <div>
                                <label className="mb-1 block text-xs font-bold uppercase tracking-wider text-slate-400">과정 (Action, 마크다운)</label>
                                <MarkdownEditor value={d.actionDetail ?? ''} onChange={(value) => updateDetailField(idx, 'actionDetail', value)} />
                              </div>
                              <div>
                                <label className="mb-1 block text-xs font-bold uppercase tracking-wider text-slate-400">성과 (Outcome, 마크다운)</label>
                                <MarkdownEditor value={d.outcome ?? ''} onChange={(value) => updateDetailField(idx, 'outcome', value)} />
                              </div>
                              <div>
                                <div className="mb-1 flex items-center justify-between gap-2">
                                  ...
                                  >
                                    {isNarrativeGenerating ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : <WandSparkles className="h-3.5 w-3.5" />}
                                    {isNarrativeGenerating ? '재작성 중...' : 'AI로 재작성'}
                                  </button>
                                </div>
                                {narrativeError && <p className="mb-1 text-[11px] font-semibold text-red-500">{narrativeError}</p>}
                                <MarkdownEditor value={d.narrative ?? ''} onChange={(value) => updateDetailField(idx, 'narrative', value)} />
                              </div>
                                  ...
                                  <div className="max-h-52 space-y-1 overflow-auto rounded-xl border border-slate-200 p-3">
                                    {selectableDetailSkills.map((s) => (
                                      <label key={s.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-slate-50">
                                        <input type="checkbox" checked={d.skillIds.includes(s.id)} onChange={() => toggleDetailSkill(idx, s.id)} />
                                        {s.name}
                                      </label>
                                    ))}
                                    {selectableDetailSkills.length === 0 && <p className="py-4 text-center text-xs font-semibold text-slate-400">검색 결과가 없습니다.</p>}
                                  </div>
                                </div>
                                <div>
                                  <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-400">이 항목의 관련 Study · {d.studyIds.length}개</label>
                                  <div className="relative mb-2">
                                    <Search className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" />
                                    <input
                                      type="search"
                                  ...
                                  <div className="max-h-52 space-y-1 overflow-auto rounded-xl border border-slate-200 p-3">
                                    {selectableDetailStudies.map((study) => (
                                      <label key={study.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-slate-50">
                                        <input type="checkbox" checked={d.studyIds.includes(study.id)} onChange={() => toggleDetailStudy(idx, study.id)} />
                                        {study.title}
                                      </label>
                                    ))}
                                    {selectableDetailStudies.length === 0 && <p className="py-4 text-center text-xs font-semibold text-slate-400">검색 결과가 없습니다.</p>}
                                  </div>
                                </div>
                              </div>
                            </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={createExpMutation.isPending || updateExpMutation.isPending} className="rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-bold text-white shadow-md hover:bg-slate-800 disabled:opacity-50">
      ...
      {!isExpFormOpen && !selectedExperience && (
        <div className="space-y-2.5">
          {filteredExperiences?.map((exp) => (
            <div key={exp.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-slate-300 hover:shadow-md">
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  <button type="button" onClick={() => handleExpDelete(exp.id)} title="삭제" className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-500 transition hover:border-red-200 hover:text-red-600">
