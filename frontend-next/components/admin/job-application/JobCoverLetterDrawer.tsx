...
export function JobCoverLetterDrawer({
    ...
    const drawerContent = (
        <div className="fixed inset-0 z-9999 overflow-hidden bg-slate-900/50 backdrop-blur-xs transition-opacity animate-in fade-in duration-200">
            <div className="absolute inset-y-0 right-0 flex max-w-full">
                <div className="w-screen max-w-6xl min-w-[768px] bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
                    {/* Drawer Header */}
                    <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 bg-slate-50/90 shrink-0">
                        <div className="flex items-center gap-3 min-w-0 pr-4">
                            ...
                            <div className="min-w-0">
                                <div className="flex items-center gap-2">
                                    <span className="rounded-md bg-indigo-100 px-2 py-0.5 text-xs font-bold text-indigo-700 whitespace-nowrap shrink-0">
                                        문항 {itemIndex + 1}
                                    </span>
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                            <span className="rounded-full bg-slate-200 px-3 py-1 text-xs font-bold text-slate-600 whitespace-nowrap">
                                {characterLimit
                            ...
                            <button
                                type="button"
                                onClick={onClose}
                                className="rounded-lg p-2 text-slate-400 hover:bg-slate-200 hover:text-slate-700 transition shrink-0"
                            >
                                <X className="h-5 w-5" />
                            </button>
                        </div>
                    ...
                    {/* Drawer Content: 2-Column Split (항상 좌 7 : 우 5 유지) */}
                    <div className="grid grid-cols-12 flex-1 overflow-hidden min-h-0">
                        {/* Left Column: Final Answer Editor (7 Columns) */}
                        <div className="col-span-7 flex flex-col border-r border-slate-200 p-6 overflow-y-auto bg-slate-50/30">
                            <div className="mb-3 flex items-center justify-between gap-2 shrink-0">
                                <label className="flex items-center gap-1.5 text-xs font-extrabold text-slate-700 whitespace-nowrap">
                            ...
                            <textarea
                                ...
                                className="w-full flex-1 resize-none rounded-xl border border-slate-200 bg-white p-4 text-sm leading-7 text-slate-800 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 focus:outline-none transition shadow-xs"
                            />

                            <div className="mt-4 flex items-center justify-between gap-3 shrink-0">
                                <p className="text-xs text-slate-400">
                                    * 오른쪽 대화창에서 지적사항을 입력하여 AI 초안을 계속 발전시킬
                                    수 있습니다.
                                </p>
                                <button
                                    type="button"
                                    ...
                                    onClick={handleSave}
                                    className="flex items-center gap-1.5 rounded-xl bg-slate-900 px-5 py-2.5 text-xs font-extrabold text-white transition hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed shadow-md whitespace-nowrap shrink-0"
                                >
                                    {isSaving ? (
                        ...
                        <div className="col-span-5 flex flex-col h-full overflow-hidden bg-white">
                            {/* Chat Header */}
                            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3.5 bg-indigo-50/40 shrink-0">
                                <div className="flex items-center gap-2 min-w-0">
                                    <div className="rounded-lg bg-indigo-600 p-1.5 text-white shrink-0">
                                        <Sparkles className="h-4 w-4" />
                                    </div>
                                    <div className="min-w-0">
                                        <h4 className="text-xs font-extrabold text-slate-800 whitespace-nowrap">
                                            AI 초안 & 지적사항 타임라인
                                        </h4>
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-1.5 rounded-lg border border-indigo-200 bg-white px-3 py-1.5 text-xs font-extrabold text-indigo-600 transition hover:bg-indigo-50 disabled:opacity-50 whitespace-nowrap shrink-0 shadow-xs"
                                >
                                    {isGenerating ? (
                            ...
                            <div className="border-t border-slate-200 p-3 bg-white shrink-0">
                                ...
                                >
                                    <input
                                        ...
                                        className="flex-1 min-w-0 rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-xs text-slate-800 focus:border-indigo-500 focus:bg-white focus:outline-none transition"
                                    />
                                    <button
                                        type="submit"
                                        disabled={isGenerating || !feedbackInput.trim()}
                                        className="flex items-center gap-1.5 rounded-xl bg-amber-600 px-3.5 py-2.5 text-xs font-extrabold text-white transition hover:bg-amber-700 disabled:opacity-40 disabled:cursor-not-allowed shrink-0 whitespace-nowrap shadow-sm"
                                    >
                                        {isGenerating ? (
                                            <Loader2 className="h-4 w-4 animate-spin shrink-0" />
                                        ) : (
