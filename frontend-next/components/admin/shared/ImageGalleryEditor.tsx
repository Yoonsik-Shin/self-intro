...
export function ImageGalleryEditor({ scope, images, onChange }: ImageGalleryEditorProps) {
  ...
  return (
    <div>
          ...
          <div key={image.objectKey} className="group relative h-24 w-24 overflow-hidden rounded-xl border border-slate-200">
            ...
            <div className="absolute inset-0 flex flex-col justify-between bg-black/0 p-1 opacity-0 transition group-hover:bg-black/40 group-hover:opacity-100">
              <div className="flex w-full justify-end">
                <button type="button" onClick={() => remove(index)} className="rounded-full bg-white/90 p-1 text-red-500">
                  <X className="h-3 w-3" />
                </button>
              </div>
              <div className="flex w-full items-center justify-center gap-1">
                <button type="button" onClick={() => move(index, -1)} disabled={index === 0} className="rounded-full bg-white/90 px-1.5 py-0.5 text-[10px] font-bold text-slate-700 disabled:opacity-30">
                  ◀
                </button>
                <button type="button" onClick={() => move(index, 1)} disabled={index === images.length - 1} className="rounded-full bg-white/90 px-1.5 py-0.5 text-[10px] font-bold text-slate-700 disabled:opacity-30">
                  ▶
                </button>
              </div>
            </div>
      ...
      <input ref={fileInputRef} type="file" accept={ACCEPTED_TYPES.join(',')} multiple className="hidden" onChange={(event) => handleFiles(event.target.files)} />
