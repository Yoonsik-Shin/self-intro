    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
      <div className="flex flex-wrap items-center gap-1 border-b border-slate-200 bg-slate-50 px-3 py-2">
        {tools.map(({ label, icon: Icon, before, after, placeholder }) => (
          <button key={label} type="button" title={label} onClick={() => insert(before, after, placeholder)} className="grid h-8 w-8 place-items-center rounded-lg text-slate-500 transition hover:bg-white hover:text-slate-900 hover:shadow-sm">
        ...
        {enableImageUpload && (
          <>
            ...
            <input ref={imageInputRef} type="file" accept={IMAGE_ACCEPT_TYPES.join(',')} className="hidden" onChange={(event) => handleImageSelected(event.target.files?.[0])} />
          </>
        )}
      ...
      <div className="grid min-h-[140px] grid-cols-1 lg:grid-cols-2">
        ...
        <article className="min-h-[140px] space-y-4 overflow-auto p-5 text-sm text-slate-700">
          {value ? <ReactMarkdown components={editorMarkdownComponents}>{value}</ReactMarkdown> : <p className="text-slate-400">작성한 Markdown이 여기에 표시됩니다.</p>}
        </article>
      </div>
    </div>
  );
