...
import {
    getSkillLevelDotClass,
    skillUsagePresentations,
    type SkillCategoryPresentation,
    ...
    return (
        <article className="flex min-h-36 flex-col rounded-xl border border-slate-200 bg-white p-3 shadow-sm transition hover:border-slate-300 hover:shadow-md">
            <div className="flex items-start justify-between gap-3">
                <p className="min-w-0 truncate font-mono text-[10px] font-bold uppercase tracking-wider text-slate-400">
                    {category.label}
                </p>
                <div className="flex shrink-0 items-center gap-1">
                    ...
                    >
                        <Star
                            className={`h-3.5 w-3.5 ${skill.isCore ? 'fill-current' : ''} ${isCoreUpdating ? 'animate-pulse' : ''}`}
                        />
                    </button>
                    <button
                        type="button"
            ...
            <div className="mt-1 flex min-w-0 items-center gap-2.5">
                ...
                <div className="min-w-0 flex-1">
                    <h4
                        className="truncate text-base font-black leading-tight text-slate-900"
                        title={skill.name}
                    >
                        {skill.name}
                    </h4>
                    <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs font-bold text-slate-500">
                        <span className="inline-flex items-center gap-1.5">
                            <span
                                className={`h-2 w-2 rounded-full ${getSkillLevelDotClass(skill.skillLevel)}`}
                            />
            ...
            <div className="mt-2.5 flex flex-wrap gap-1.5">
                <span
                    className={`inline-flex items-center gap-1 rounded border px-1.5 py-0.5 text-[11px] font-bold ${usage.className}`}
                >
            ...
            <p
                className={`mt-2.5 line-clamp-2 text-xs font-medium leading-4 ${skill.comment ? 'text-slate-500' : 'italic text-slate-300'}`}
            >
                {skill.comment || '설명이 등록되지 않았습니다.'}
            </p>
        </article>
    );
}
