...
export function SkillGroupSection({ category, skills, onEdit, onDelete, onToggleCore, updatingCoreSkillId }: SkillGroupSectionProps) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded border border-slate-200 bg-white px-1.5 py-0.5 text-xs font-bold text-slate-500">{skills.length}개</span>
            </div>
            <p className="mt-0.5 truncate text-xs font-medium text-slate-500">{category.description}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-2.5 p-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
        {skills.map((skill) => (
          <SkillManagementCard key={skill.id} skill={skill} category={category} onEdit={onEdit} onDelete={onDelete} onToggleCore={onToggleCore} isCoreUpdating={updatingCoreSkillId === skill.id} />
        ))}
      </div>
    </section>
  );
