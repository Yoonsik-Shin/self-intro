...
import { BookOpen, Check, Pencil, Plus, RefreshCw, Search, Trash2, WandSparkles } from 'lucide-react';
...
export function StudyManagement() {
  ...
  const { data: studyCategories } = useQuery({ queryKey: ['studyCategories'], queryFn: studyApi.categories });
  ...
  const selectableStudyExperiences = useMemo(() => {
    ...
    return (experiencesList ?? []).filter((experience) => experience.title.toLowerCase().includes(keyword) || experience.type.toLowerCase().includes(keyword));
  ...
  const selectableStudyExperienceDetails = useMemo(() => {
    ...
    return (experiencesList ?? [])
      .map((experience) => ({
        experience,
        details: experience.details.filter((detail) => !keyword || experience.title.toLowerCase().includes(keyword) || detail.content.toLowerCase().includes(keyword)),
      }))
  ...
  const selectableRelatedStudies = useMemo(() => {
    ...
    return (studies ?? []).filter((study) => {
      ...
      return !keyword || study.title.toLowerCase().includes(keyword) || study.slug.toLowerCase().includes(keyword);
    });
  ...
  const updateStudyMutation = useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: StudyForm }) => studyApi.update(id, toStudyRequest(payload)),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['learning'] });
      queryClient.invalidateQueries({ queryKey: ['studies'] });
  ...
  const applyStudyAiSuggestion = (suggestion: StudySuggestion) => {
    if ((studyForm.title.trim() || studyForm.summary.trim() || studyForm.contentMarkdown.trim()) && !window.confirm('현재 작성한 제목·요약·본문을 AI 초안으로 바꾸시겠습니까?')) return;
    setStudyForm({
      ...studyForm,
  ...
  return (
    <div className="space-y-6">
            ...
            {[{ slug: 'ALL', name: '전체' }, ...(studyCategories ?? [])].map((category) => (
              <button
                ...
                className={`px-3 py-1.5 text-sm font-bold rounded-lg transition ${
                  studyFilter === category.slug ? 'bg-slate-900 text-white shadow-sm' : 'bg-slate-50 text-slate-500 hover:bg-slate-100 hover:text-slate-800 border border-slate-100'
                }`}
              >
                {category.name}
              </button>
        ...
        <form onSubmit={handleStudySubmit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                  ...
                  <div ref={studyAiChatRef} className="mt-4 max-h-80 space-y-2.5 overflow-y-auto rounded-xl border border-violet-100 bg-white p-3">
                    {studyAiStages.map((stageItem) => (
                      <AiStageBubble
                        ...
                        extra={stageItem.stage === 1 && studyAiFactCount > 0 ? <p className="mt-2 text-[11px] font-bold text-violet-600">검증된 사실 {studyAiFactCount}개</p> : undefined}
                      />
                    ))}
                    {studyAiError && <p className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">{studyAiError}</p>}
                  </div>
                )}
                        ...
                        {suggestion.tagNames.length > 0 && (
                          <div className="mt-2 flex flex-wrap gap-1">
                            {suggestion.tagNames.map((tag) => (
                              <span key={tag} className="rounded bg-blue-50 px-1.5 py-1 text-[10px] font-bold text-blue-700">
                                {tag}
                              </span>
                            ))}
                          </div>
                        )}
                        <div className="mt-3 max-h-56 overflow-auto whitespace-pre-wrap rounded-lg bg-slate-50 px-3 py-2 text-[11px] leading-relaxed text-slate-600">{suggestion.contentMarkdown}</div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div>
              ...
              <div className="max-h-44 space-y-1 overflow-auto rounded-xl border border-slate-200 p-3">
                  ...
                  <label key={skill.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-slate-50">
                    <input
                      ...
                      onChange={(event) =>
                        setStudyForm({
                          ...studyForm,
                          skillIds: event.target.checked ? [...studyForm.skillIds, skill.id] : studyForm.skillIds.filter((id) => id !== skill.id),
                        })
                      }
                    />
                    {skill.name}
                ...
                {selectableStudySkills.length === 0 && <p className="py-4 text-center text-xs font-semibold text-slate-400">검색 결과가 없습니다.</p>}
              </div>
            </div>
            <div>
              ...
              <div className="max-h-44 space-y-1 overflow-auto rounded-xl border border-slate-200 p-3">
                  ...
                  <label key={experience.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-slate-50">
                    <input
                      ...
                      onChange={(event) =>
                        setStudyForm({
                          ...studyForm,
                          experienceIds: event.target.checked ? [...studyForm.experienceIds, experience.id] : studyForm.experienceIds.filter((id) => id !== experience.id),
                        })
                      }
                    />
                ...
                {selectableStudyExperiences.length === 0 && <p className="py-4 text-center text-xs font-semibold text-slate-400">검색 결과가 없습니다.</p>}
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="max-h-52 space-y-3 overflow-auto rounded-xl border border-slate-200 p-3">
                    ...
                    <label key={detail.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-slate-50">
                      <input
                        ...
                        onChange={(event) =>
                          setStudyForm({
                            ...studyForm,
                            experienceDetailIds: event.target.checked ? [...studyForm.experienceDetailIds, detail.id] : studyForm.experienceDetailIds.filter((id) => id !== detail.id),
                          })
                        }
                      />
                      {detail.content}
              ...
              {selectableStudyExperienceDetails.length === 0 && <p className="py-4 text-center text-xs font-semibold text-slate-400">검색 결과가 없습니다.</p>}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="max-h-52 space-y-2 overflow-auto rounded-xl border border-slate-200 p-3">
                  ...
                  <div key={study.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-slate-50">
                    <input
                      ...
                      onChange={(event) =>
                        setStudyForm({
                          ...studyForm,
                          relatedStudies: event.target.checked ? [...studyForm.relatedStudies, { studyId: study.id, type: 'RELATED' }] : studyForm.relatedStudies.filter((item) => item.studyId !== study.id),
                        })
                      }
                    />
                    ...
                    {relation && (
                      <select
                        ...
                        onChange={(event) =>
                          setStudyForm({
                            ...studyForm,
                            relatedStudies: studyForm.relatedStudies.map((item) => (item.studyId === study.id ? { ...item, type: event.target.value as typeof item.type } : item)),
                          })
                        }
              ...
              {selectableRelatedStudies.length === 0 && <p className="py-4 text-center text-xs font-semibold text-slate-400">검색 결과가 없습니다.</p>}
            </div>
          </div>

          <div>
            <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-400">이미지</label>
            <ImageGalleryEditor scope="STUDY_GALLERY" images={studyForm.images} onChange={(images) => setStudyForm({ ...studyForm, images })} />
          </div>

          <div>
            <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-400">Markdown 본문</label>
            <MarkdownEditor value={studyForm.contentMarkdown} onChange={(contentMarkdown) => setStudyForm({ ...studyForm, contentMarkdown })} enableImageUpload />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={createStudyMutation.isPending || updateStudyMutation.isPending} className="rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-bold text-white shadow-md hover:bg-slate-800 disabled:opacity-50">
      ...
      {!isStudyFormOpen && selectedStudy && <StudyDetailPanel study={selectedStudy} onBack={() => setSelectedStudyId(null)} onEdit={openStudyEditor} onDelete={handleStudyDelete} />}
        ...
        <div className="space-y-2.5">
          ...
          {!isStudyListLoading && !isStudyListError && filteredStudies?.length === 0 && (
            <div className="flex min-h-48 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white/70 px-6 py-12 text-center">
              ...
              <p className="mt-3 text-base font-black text-slate-700">{studies?.length === 0 ? '아직 등록된 Study가 없습니다.' : '조건에 맞는 Study가 없습니다.'}</p>
              <p className="mt-1 text-sm font-medium text-slate-400">{studies?.length === 0 ? '새 글 작성을 눌러 첫 번째 학습 기록을 추가해 보세요.' : '카테고리나 검색어를 변경해 보세요.'}</p>
            </div>
          )}
          ...
          {!isStudyListLoading &&
            ...
            filteredStudies?.map((study) => (
              <div key={study.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-slate-300 hover:shadow-md">
                <div className="flex items-center justify-between gap-4">
                  <button type="button" className="min-w-0 flex-1 text-left" onClick={() => setSelectedStudyId(study.id)}>
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    <button type="button" onClick={() => openStudyEditor(study)} title="수정" className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-500 transition hover:border-slate-300 hover:text-slate-900">
