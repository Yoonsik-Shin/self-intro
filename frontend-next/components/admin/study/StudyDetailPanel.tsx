...
import { ArrowLeft, BookOpen, Briefcase, CalendarDays, Link2, Pencil, Tags, Trash2, Wrench } from 'lucide-react';
      ...
      <div className="border-b border-slate-200 bg-slate-50/70 px-5 py-4 sm:px-7">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button type="button" onClick={() => onEdit(study)} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-600 transition hover:border-slate-300 hover:text-slate-950">
              <Pencil className="h-3.5 w-3.5" />
              수정
            </button>
            <button type="button" onClick={() => onDelete(study.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-red-100 bg-white px-3 py-2 text-sm font-bold text-red-500 transition hover:border-red-200 hover:bg-red-50">
        ...
        <div className="mt-6 max-w-5xl">
          <div className="flex flex-wrap items-center gap-2 text-xs font-bold text-slate-500">
            ...
            <span className={`rounded-full px-2.5 py-1 ${study.status === 'PUBLISHED' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{study.status === 'PUBLISHED' ? '공개' : '초안'}</span>
