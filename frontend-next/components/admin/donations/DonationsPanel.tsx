...
export function DonationsPanel() {
  ...
  const { data: donationSummary, isLoading: isDonationLoading } = useQuery({ queryKey: ['donations', 'admin'], queryFn: donationApi.adminList });
  const { data: donationConfig } = useQuery({ queryKey: ['donationConfig'], queryFn: donationApi.config });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b border-slate-200 pb-3">
        ...
        <label className="flex cursor-pointer items-center gap-3">
          ...
          >
            <span className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow transition-all ${donationConfig?.enabled ? 'left-6' : 'left-1'}`} />
          </button>
        </label>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {[
          ...
          <div key={label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-3 text-3xl font-black tracking-tight text-slate-950">{isDonationLoading || value === undefined ? '—' : `${value.toLocaleString()}${suffix}`}</p>
          </div>
        ))}
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {(donationSummary?.donations ?? []).map((donation) => (
                <Fragment key={donation.id}>
                  <tr className="cursor-pointer text-slate-600 transition hover:bg-slate-50" onClick={() => setExpandedDonationId(expandedDonationId === donation.id ? null : donation.id)}>
                    <td className="px-5 py-3 font-semibold text-slate-700 whitespace-nowrap">
                      <span className="mr-2 inline-block text-slate-400">{expandedDonationId === donation.id ? '▾' : '▸'}</span>
                    ...
                    <td className="px-5 py-3">
                      <span
                        className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-extrabold ${
                          donation.status === 'PAID' ? 'bg-emerald-50 text-emerald-600' : donation.status === 'CANCELED' ? 'bg-amber-50 text-amber-600' : donation.status === 'FAILED' ? 'bg-rose-50 text-rose-600' : 'bg-slate-100 text-slate-500'
                        }`}
                      >
                        {donation.status === 'PAID' ? '결제완료' : donation.status === 'CANCELED' ? '취소됨' : donation.status === 'FAILED' ? '실패' : '대기'}
                      </span>
                    </td>
                          ...
                          <ol className="space-y-2">
                            {donationEvents.map((event) => (
                              <li key={event.id} className="flex items-baseline gap-3 text-sm">
                                <span className="whitespace-nowrap font-mono text-xs text-slate-400">{event.createdAt.replace('T', ' ').slice(0, 19)}</span>
                                <span
                                  className={`font-extrabold ${
                                    event.eventType === 'PAID' ? 'text-emerald-600' : event.eventType === 'CANCELED' ? 'text-amber-600' : event.eventType === 'PAY_FAILED' || event.eventType === 'CALLBACK_REJECTED' ? 'text-rose-600' : 'text-slate-700'
                                  }`}
                                >
                                  {DONATION_EVENT_LABELS[event.eventType]}
                                </span>
                                <span className="rounded bg-slate-200 px-1.5 py-0.5 text-xs font-bold text-slate-600">{DONATION_ACTOR_LABELS[event.actor]}</span>
                                {event.payState && <span className="text-xs text-slate-400">pay_state={event.payState}</span>}
                                {event.detail && (
