...
import type { VisitorDaily, VisitorHourly } from '@/lib/api/types';
    ...
    <div className="flex items-center justify-end gap-4 pb-2">
      {SERIES.map((series) => (
        <span key={series.key} className="flex items-center gap-1.5 text-xs font-semibold text-slate-500">
          {variant === 'line' ? <span className="h-0.5 w-4 rounded-full" style={{ backgroundColor: series.color }} /> : <span className="h-2.5 w-2.5 rounded-[3px]" style={{ backgroundColor: series.color }} />}
          {series.label}
        </span>
      ))}
    </div>
...
function ChartTooltip({ left, title, values }: { left: number; title: string; values: Record<SeriesKey, number> }) {
  return (
    <div className="pointer-events-none absolute top-2 z-10 -translate-x-1/2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-lg" style={{ left }}>
      ...
      <div className="mt-1 space-y-0.5">
        {SERIES.map((series) => (
          <div key={series.key} className="flex items-center gap-2">
            ...
            <span className="text-sm font-black tabular-nums text-slate-900">{values[series.key].toLocaleString()}</span>
...
export function VisitorTrendChart({ data }: { data: VisitorDaily[] }) {
  ...
  const maxValue = useMemo(() => data.reduce((max, day) => Math.max(max, day.visitors, day.pageViews), 0), [data]);
  ...
  const xFor = (index: number) => PADDING.left + (data.length <= 1 ? innerWidth / 2 : (index * innerWidth) / (data.length - 1));
  ...
  return (
    <div>
      ...
      <div ref={ref} className="relative">
        {width > 0 && (
          <svg width={width} height={CHART_HEIGHT} role="img" aria-label="최근 14일 순 방문자 및 조회 수 추이" onPointerMove={(event) => moveHover(event.clientX, event.currentTarget)} onPointerLeave={() => setHoverIndex(null)}>
            {ticks.map((tick) => (
              <g key={tick}>
                <line x1={PADDING.left} x2={width - PADDING.right} y1={yFor(tick)} y2={yFor(tick)} stroke={tick === 0 ? BASELINE_COLOR : GRID_COLOR} strokeWidth={1} />
                <text x={PADDING.left - 8} y={yFor(tick) + 3.5} textAnchor="end" fontSize={11} fill={TICK_COLOR} className="tabular-nums">
                  {tick.toLocaleString()}
                </text>
              </g>
            ))}
            {data.map((day, index) =>
              index % 2 === 0 ? (
                <text key={day.date} x={xFor(index)} y={CHART_HEIGHT - 8} textAnchor="middle" fontSize={11} fill={TICK_COLOR} className="tabular-nums">
                  {formatDayTick(day.date)}
                </text>
              ) : null,
            )}
            {hoverIndex !== null && hasData && <line x1={xFor(hoverIndex)} x2={xFor(hoverIndex)} y1={PADDING.top} y2={PADDING.top + innerHeight} stroke={BASELINE_COLOR} strokeWidth={1} />}
            {hasData &&
              SERIES.map((series) => (
                <g key={series.key}>
                  <path d={data.map((day, index) => `${index === 0 ? 'M' : 'L'}${xFor(index)},${yFor(day[series.key])}`).join(' ')} fill="none" stroke={series.color} strokeWidth={2} strokeLinejoin="round" strokeLinecap="round" />
                  {data.map((day, index) => (
                    <circle key={day.date} cx={xFor(index)} cy={yFor(day[series.key])} r={hoverIndex === index ? 5 : 4} fill={series.color} stroke="#ffffff" strokeWidth={2} />
                  ))}
                </g>
              ))}
            {data.map((day, index) => (
...
const roundedTopBarPath = (x: number, y: number, barWidth: number, barHeight: number) => {
  ...
  return [`M${x},${y + barHeight}`, `L${x},${y + radius}`, `Q${x},${y} ${x + radius},${y}`, `L${x + barWidth - radius},${y}`, `Q${x + barWidth},${y} ${x + barWidth},${y + radius}`, `L${x + barWidth},${y + barHeight}`, 'Z'].join(' ');
};
...
export function VisitorHourlyChart({ data }: { data: VisitorHourly[] }) {
  ...
  const maxValue = useMemo(() => data.reduce((max, slot) => Math.max(max, slot.visitors, slot.pageViews), 0), [data]);
  const { ticks, domainMax } = useMemo(() => buildScale(maxValue), [maxValue]);
  ...
  const yFor = (value: number) => PADDING.top + innerHeight * (1 - value / domainMax);
  const barX = (index: number, seriesIndex: number) => xForSlot(index) + (slotWidth - groupWidth) / 2 + seriesIndex * (barWidth + 2);
  ...
  return (
    <div>
      ...
      <div ref={ref} className="relative">
        {width > 0 && (
          <svg width={width} height={CHART_HEIGHT} role="img" aria-label="오늘 시간대별 순 방문자 및 조회 수" onPointerLeave={() => setHoverIndex(null)}>
            {ticks.map((tick) => (
              <g key={tick}>
                <line x1={PADDING.left} x2={width - PADDING.right} y1={yFor(tick)} y2={yFor(tick)} stroke={tick === 0 ? BASELINE_COLOR : GRID_COLOR} strokeWidth={1} />
                <text x={PADDING.left - 8} y={yFor(tick) + 3.5} textAnchor="end" fontSize={11} fill={TICK_COLOR} className="tabular-nums">
            ...
            {data.map((slot) =>
              slot.hour % 3 === 0 ? (
                <text key={slot.hour} x={xForSlot(slot.hour) + slotWidth / 2} y={CHART_HEIGHT - 8} textAnchor="middle" fontSize={11} fill={TICK_COLOR} className="tabular-nums">
                  {slot.hour}시
                </text>
              ) : null,
            )}
            {hoverIndex !== null && hasData && <rect x={xForSlot(hoverIndex)} y={PADDING.top} width={slotWidth} height={innerHeight} fill="#f1f5f9" />}
            {hasData &&
              data.map((slot, index) =>
                SERIES.map((series, seriesIndex) => {
                  ...
                  return <path key={`${slot.hour}-${series.key}`} d={roundedTopBarPath(barX(index, seriesIndex), barTop, barWidth, PADDING.top + innerHeight - barTop)} fill={series.color} />;
                }),
              )}
