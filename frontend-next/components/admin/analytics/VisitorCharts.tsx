...
export function VisitorTrendChart({ data }: { data: VisitorDaily[] }) {
    ...
    const xFor = (index: number) =>
        PADDING.left +
        (data.length <= 1 ? innerWidth / 2 : (index * innerWidth) / (data.length - 1));
            ...
            <div ref={ref} className="relative">
                                ...
                                <g key={series.key}>
                                    <path
                                        d={data
                                            .map(
                                                (day, index) =>
                                                    `${index === 0 ? 'M' : 'L'}${xFor(index)},${yFor(day[series.key])}`
                                            )
                                            .join(' ')}
                                        fill="none"
                                        stroke={series.color}
                ...
                {hoverIndex !== null && hasData && (
                    <ChartTooltip
                        ...
                        values={{
                            visitors: data[hoverIndex].visitors,
                            pageViews: data[hoverIndex].pageViews,
                        }}
                    />
                )}
            </div>
        ...
        <div>
            ...
            <div ref={ref} className="relative">
                        ...
                        {peak !== null && hoverIndex === null && (
                            <text
                                x={
                                    barX(
                                        peak.index,
                                        SERIES.findIndex((series) => series.key === peak.seriesKey)
                                    ) +
                                    barWidth / 2
                                }
                                y={yFor(peak.value) - 6}
                ...
                {hoverIndex !== null && hasData && (
                    <ChartTooltip
                        left={Math.min(
                            Math.max(xForSlot(hoverIndex) + slotWidth / 2, 90),
