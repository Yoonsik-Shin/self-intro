  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <div key={label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-3 text-3xl font-black tracking-tight text-slate-950">{isVisitorSummaryLoading || value === undefined ? '—' : value.toLocaleString()}</p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <div>
            ...
            <p className="mt-0.5 text-xs font-medium text-slate-400">{visitorDateRange.to} · 0시 ~ 23시</p>
          </div>
          <Clock className="h-5 w-5 text-slate-400" />
        </div>
        <div className="px-5 pb-5 pt-4">{isVisitorHourlyLoading ? <p className="py-10 text-center text-sm font-semibold text-slate-400">통계를 불러오는 중입니다.</p> : <VisitorHourlyChart data={visitorHourly} />}</div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="border-b border-slate-200 px-5 pb-5 pt-4">{isVisitorDailyLoading ? <p className="py-10 text-center text-sm font-semibold text-slate-400">통계를 불러오는 중입니다.</p> : <VisitorTrendChart data={visitorDaily} />}</div>
