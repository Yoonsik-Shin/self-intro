    ...
    <aside className="block print:hidden w-full sticky top-24 self-start">
      <div
        className={`relative rounded-2xl border border-slate-200/80 bg-white/80 p-2 shadow-md backdrop-blur-md min-[900px]:flex min-[900px]:flex-col min-[900px]:border-l-4 min-[900px]:border-l-slate-300 ${
          isCollapsed ? 'min-[900px]:gap-3 min-[900px]:px-1.5 min-[900px]:py-3' : 'min-[900px]:gap-4 min-[900px]:px-5 min-[900px]:py-4'
        }`}
      >
        <button
          type="button"
          ...
          className={`z-20 hidden items-center justify-center border border-slate-200 bg-white text-slate-400 transition-colors hover:border-slate-300 hover:bg-slate-50 hover:text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300 min-[900px]:flex ${
            isCollapsed ? 'relative mx-auto h-8 w-8 shrink-0 rounded-full shadow-sm' : 'absolute -right-[11px] top-7 !m-0 h-10 w-5 rounded-r-lg border-l-0 bg-white/95 shadow-[3px_1px_6px_-3px_rgba(15,23,42,0.35)]'
          }`}
        ...
        <div className={`hidden relative pl-4 before:absolute before:top-2.5 before:bottom-2.5 before:left-[4px] before:w-[2px] before:bg-slate-200 ${isCollapsed ? '' : 'min-[900px]:block'}`}>
          {sections.map((step) => (
            <button key={step.id} onClick={() => scrollToSection(step.id)} className="group flex items-start gap-3 w-full text-left py-2.5 relative transition-all duration-200">
              <div
                className={`absolute left-[-15px] top-[14px] w-2 h-2 rounded-full border border-white transition-all duration-300 z-10 ${
                  activeSectionId === step.id ? 'bg-slate-900 scale-125 ring-4 ring-slate-200' : 'bg-slate-300 group-hover:bg-slate-500'
                }`}
              />
              <span className={`rounded-lg px-2 py-1 text-sm font-bold leading-tight transition-all duration-200 ${activeSectionId === step.id ? 'bg-slate-100 text-slate-900 font-extrabold' : 'text-slate-600 hover:bg-slate-100/60 hover:text-slate-950'}`}>
                {step.label}
              </span>
            </button>
          ))}
        ...
        <div className={`relative flex flex-col items-center gap-2 py-1.5 before:absolute before:bottom-5 before:top-5 before:left-1/2 before:w-px before:-translate-x-1/2 before:bg-slate-200 ${isCollapsed ? 'min-[900px]:flex' : 'min-[900px]:hidden'}`}>
          {sections.map((step) => {
            ...
            return (
              <button
                ...
                className={`relative z-10 grid h-8 w-8 place-items-center rounded-full border transition-all duration-200 ${
                  activeSectionId === step.id ? 'border-slate-300 bg-slate-900 text-white shadow-sm shadow-slate-800/20 ring-4 ring-slate-200' : 'border-slate-200 bg-white text-slate-500 hover:border-slate-300 hover:text-slate-900'
                }`}
              >
        ...
        <hr className={`hidden border-slate-100 ${isCollapsed ? '' : 'min-[900px]:block'}`} />
        ...
        >
          ...
          <span className={`hidden ${isCollapsed ? '' : 'min-[900px]:inline'}`}>위로 가기</span>
        </button>
      </div>
    </aside>
  );
