...
const pages = [
  ...
  { href: '/architecture', label: '시스템 아키텍처', shortLabel: '시스템 아키텍처', icon: Terminal },
];

function isActivePage(pathname: string, href: string): boolean {
  if (href === '/') return pathname === '/';
  if (href === '/experience') return pathname === '/experience' || pathname.startsWith('/experience-detail/');
    ...
    <header className="sticky top-0 z-30 border-b border-slate-200/70 bg-white/90 py-2 shadow-sm backdrop-blur-xl print:hidden">
      <div className="mx-auto flex h-12 max-w-[1500px] items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
        <div className="flex min-w-0 items-center gap-6">
          <Link href="/" className="flex shrink-0 items-center text-left focus:outline-none hover:opacity-90 transition" title="소개 페이지로 이동">
            <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-gradient-to-tr from-slate-900 to-slate-950 text-sm font-black text-white shadow-md shadow-slate-800/20">YS</div>
          </Link>

          <nav aria-label="페이지 네비게이션" className="hidden min-w-0 items-center gap-5 overflow-x-auto scrollbar-none min-[900px]:flex">
            {pages.map((page) => {
              ...
              return (
                <Link
                  ...
                  className={`relative inline-flex h-12 shrink-0 items-center gap-2 px-1 text-sm font-black transition-colors duration-200 after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-full after:origin-center after:rounded-full after:bg-slate-900 after:transition-transform after:duration-200 ${
                    isActive ? 'text-slate-950 after:scale-x-100' : 'text-slate-500 after:scale-x-0 hover:text-slate-900'
                  }`}
                  title={page.label}
                >
              ...
              return (
                <Link
                  ...
                  className={`relative flex w-full items-center gap-3 px-3 py-2.5 text-left text-sm font-black transition-colors duration-200 after:absolute after:bottom-0 after:left-3 after:right-3 after:h-0.5 after:rounded-full after:bg-slate-900 ${
                    isActive ? 'text-slate-950 after:opacity-100' : 'text-slate-600 after:opacity-0 hover:text-slate-900'
                  }`}
                >
                  <Icon className="h-4 w-4" />
