...
export function StudyDetailClient({ study }: Props) {
  ...
  const hasRelated = study.experiences.length > 0 || study.experienceDetails.length > 0 || study.relatedStudies.length > 0;
        ...
        <aside className="block w-full sticky top-24 self-start">
          ...
          >
            ...
            <div className={`hidden ${isNavCollapsed ? '' : 'min-[900px]:block min-[900px]:pr-12'}`}>
              ...
              <p className="mt-0.5 text-sm leading-none text-slate-500">이 학습과 연관된 이력 정보입니다.</p>
            </div>

            <div className={`hidden space-y-4 ${isNavCollapsed ? '' : 'min-[900px]:block'}`}>
              {study.experiences.length > 0 && (
                <div>
                  <h4 className="mb-1 text-xs font-black uppercase text-slate-400">관련 프로젝트·경력</h4>
                  <div className="space-y-1.5">
                    {study.experiences.map((experience) => (
                      <p key={experience.id} className="text-xs leading-normal text-slate-600">
                        <span className="mr-1.5 rounded bg-slate-100 px-1 py-0.5 font-mono text-[10px] font-bold text-slate-400">{experience.type}</span>
                        {experience.title}
                      </p>
                    ))}
                  </div>
              ...
              {study.experienceDetails.length > 0 && (
                <div>
                  <h4 className="mb-1 text-xs font-black uppercase text-slate-400">관련 경력 항목</h4>
                  <div className="space-y-1.5">
              ...
              {study.relatedStudies.length > 0 && (
                <div>
                  <h4 className="mb-1 text-xs font-black uppercase text-slate-400">관련 Study</h4>
                  <div className="space-y-1.5">
              ...
              {!hasRelated && <p className="text-xs font-bold italic text-slate-400">연결된 이력 항목이 없습니다.</p>}
            </div>
