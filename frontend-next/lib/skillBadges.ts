export type SkillBadgeOption = {
    key: string;
    label: string;
    color: string;
    aliases: string[];
    // Simple Icons(cdn.simpleicons.org)에 브랜드가 아예 없는 경우(주로 AWS/Microsoft/Java/Playwright/OpenAI
    // 등 상표 이슈로 빠진 브랜드)의 대체 아이콘. 있으면 이 URL을 그대로 쓰고 색상 파라미터는 적용하지 않는다
    // (이미 실제 브랜드 컬러의 완성된 SVG라 단색화하면 오히려 원본보다 어색해진다).
    iconUrl?: string;
};

export const skillBadgeOptions: SkillBadgeOption[] = [
    {
        key: 'java',
        label: 'Java',
        color: 'ED8B00',
        aliases: ['java'],
        iconUrl: 'https://api.iconify.design/logos:java.svg',
    },
    { key: 'typescript', label: 'TypeScript', color: '3178C6', aliases: ['typescript'] },
    { key: 'python', label: 'Python', color: '3776AB', aliases: ['python'] },
    { key: 'nodedotjs', label: 'Node.js', color: '5FA04E', aliases: ['nodejs', 'node.js'] },
    { key: 'html5', label: 'HTML5', color: 'E34F26', aliases: ['html', 'html/css', 'html5'] },
    { key: 'css', label: 'CSS', color: '663399', aliases: ['css', 'css3'] },
    {
        key: 'springboot',
        label: 'Spring Boot',
        color: '6DB33F',
        aliases: ['spring', 'spring boot'],
    },
    {
        key: 'spring',
        label: 'Spring Data JPA',
        color: '6DB33F',
        aliases: ['spring data jpa', 'jpa'],
    },
    {
        key: 'springsecurity',
        label: 'Spring Security',
        color: '6DB33F',
        aliases: ['spring security'],
    },
    { key: 'fastapi', label: 'FastAPI', color: '009688', aliases: ['fastapi'] },
    { key: 'nestjs', label: 'NestJS', color: 'E0234E', aliases: ['nestjs'] },
    { key: 'express', label: 'Express', color: '000000', aliases: ['express', 'expressjs'] },
    { key: 'react', label: 'React', color: '61DAFB', aliases: ['react', 'reactjs'] },
    { key: 'django', label: 'Django', color: '092E20', aliases: ['django'] },
    { key: 'mongodb', label: 'MongoDB', color: '47A248', aliases: ['mongodb'] },
    { key: 'redis', label: 'Redis', color: 'FF4438', aliases: ['redis'] },
    { key: 'mysql', label: 'MySQL / SQL', color: '4479A1', aliases: ['mysql', 'sql'] },
    {
        key: 'postgresql',
        label: 'PostgreSQL',
        color: '4169E1',
        aliases: ['postgresql', 'postgres'],
    },
    {
        key: 'grpc',
        label: 'gRPC',
        color: '244C5A',
        aliases: ['grpc'],
        iconUrl: 'https://api.iconify.design/logos:grpc.svg',
    },
    {
        key: 'microsoftazure',
        label: 'Microsoft Azure',
        color: '0078D4',
        aliases: ['azure', 'cosmos db', 'bicep', 'azure functions'],
        iconUrl: 'https://api.iconify.design/logos:microsoft-azure.svg',
    },
    {
        key: 'microsoftexcel',
        label: 'Microsoft Excel',
        color: '217346',
        aliases: ['excel'],
        iconUrl: 'https://api.iconify.design/vscode-icons:file-type-excel.svg',
    },
    {
        key: 'microsoftaccess',
        label: 'Microsoft Access',
        color: 'A4373A',
        aliases: ['access'],
        iconUrl: 'https://api.iconify.design/vscode-icons:file-type-access.svg',
    },
    { key: 'flyway', label: 'Flyway', color: 'CC0200', aliases: ['flyway'] },
    {
        key: 'playwright',
        label: 'Playwright',
        color: '2EAD33',
        aliases: ['playwright'],
        iconUrl: 'https://api.iconify.design/logos:playwright.svg',
    },
    { key: 'n8n', label: 'n8n', color: 'EA4B71', aliases: ['n8n'] },
    { key: 'nginx', label: 'NGINX', color: '009639', aliases: ['nginx'] },
    { key: 'docker', label: 'Docker', color: '2496ED', aliases: ['docker', 'docker compose'] },
    { key: 'grafana', label: 'Grafana', color: 'F46800', aliases: ['grafana', 'loki', 'alloy'] },
    { key: 'kubernetes', label: 'Kubernetes', color: '326CE5', aliases: ['kubernetes'] },
    { key: 'argo', label: 'ArgoCD', color: 'EF7B4D', aliases: ['argocd', 'argo cd', 'argo'] },
    {
        key: 'hibernate',
        label: 'JPA/Hibernate',
        color: '59666C',
        aliases: ['jpa', 'hibernate', 'jpa/hibernate'],
    },
    {
        key: 'amazonecs',
        label: 'AWS ECS',
        color: 'FF9900',
        aliases: ['aws ecs', 'amazon ecs'],
        iconUrl: 'https://api.iconify.design/logos:aws-ecs.svg',
    },
    {
        key: 'amazonsqs',
        label: 'Amazon SQS',
        color: 'FF4F8B',
        aliases: ['amazon sqs', 'aws sqs', 'sqs'],
        iconUrl: 'https://api.iconify.design/logos:aws-sqs.svg',
    },
    {
        key: 'amazonec2',
        label: 'AWS EC2',
        color: 'FF9900',
        aliases: ['aws ec2', 'amazon ec2', 'ec2'],
        iconUrl: 'https://api.iconify.design/logos:aws-ec2.svg',
    },
    {
        key: 'amazonaurora',
        label: 'Aurora RDS',
        color: 'FF9900',
        aliases: ['aurora rds', 'amazon aurora', 'aurora', 'rds'],
        iconUrl: 'https://api.iconify.design/logos:aws-aurora.svg',
    },
    {
        key: 'amazons3',
        label: 'AWS S3',
        color: 'FF9900',
        aliases: ['aws s3', 'amazon s3', 's3'],
        iconUrl: 'https://api.iconify.design/logos:aws-s3.svg',
    },
    {
        key: 'amazonelasticache',
        label: 'AWS ElastiCache',
        color: 'FF9900',
        aliases: ['elasticache', 'aws elasticache', 'amazon elasticache', 'elastic cache'],
        iconUrl: 'https://api.iconify.design/logos:aws-elasticache.svg',
    },
    {
        key: 'amazoncloudfront',
        label: 'AWS CloudFront',
        color: 'FF9900',
        aliases: ['cloudfront', 'aws cloudfront', 'amazon cloudfront'],
        iconUrl: 'https://api.iconify.design/logos:aws-cloudfront.svg',
    },
    {
        key: 'amazonsns',
        label: 'Amazon SNS',
        color: 'FF9900',
        aliases: ['sns', 'aws sns', 'amazon sns'],
        iconUrl: 'https://api.iconify.design/logos:aws-sns.svg',
    },
    {
        key: 'awslambda',
        label: 'AWS Lambda',
        color: 'FF9900',
        aliases: ['lambda', 'aws lambda'],
        iconUrl: 'https://api.iconify.design/logos:aws-lambda.svg',
    },
    { key: 'datadog', label: 'Datadog', color: '632CA6', aliases: ['datadog'] },
    { key: 'prometheus', label: 'Prometheus', color: 'E6522C', aliases: ['prometheus'] },
    { key: 'apachekafka', label: 'Apache Kafka', color: '231F20', aliases: ['kafka'] },
    {
        key: 'openai',
        label: 'OpenAI / LLM',
        color: '412991',
        aliases: ['openai', 'azure openai', 'llm', 'rag'],
        iconUrl: 'https://api.iconify.design/logos:openai-icon.svg',
    },
    { key: 'langchain', label: 'LangChain', color: '1C3C3C', aliases: ['langchain', 'langgraph'] },
    {
        key: 'microsoftteams',
        label: 'Microsoft Teams',
        color: '6264A7',
        aliases: ['teams', 'teams sdk'],
        iconUrl: 'https://api.iconify.design/logos:microsoft-teams.svg',
    },
    { key: 'githubactions', label: 'GitHub Actions', color: '2088FF', aliases: ['github actions'] },
    { key: 'git', label: 'Git', color: 'F05032', aliases: ['git'] },
];

const normalizeSkillName = (value: string) => value.trim().toLowerCase().replace(/\s+/g, ' ');

export function recommendSkillBadge(skillName: string): SkillBadgeOption | undefined {
    const normalized = normalizeSkillName(skillName);
    return skillBadgeOptions.find((option) => option.aliases.includes(normalized));
}

export function findSkillBadge(key?: string | null): SkillBadgeOption | undefined {
    return skillBadgeOptions.find((option) => option.key === key);
}

export function resolveSkillBadge(
    skillName: string,
    badgeKey?: string | null,
    badgeColor?: string | null
) {
    const recommended = recommendSkillBadge(skillName);
    const key = badgeKey?.trim() || recommended?.key;
    if (!key) return undefined;
    const option = findSkillBadge(key);
    const color = badgeColor?.trim() || option?.color || recommended?.color || '64748B';
    return {
        key,
        color: /^[0-9A-Fa-f]{6}$/.test(color) ? color.toUpperCase() : '64748B',
        iconUrl: option?.iconUrl ?? recommended?.iconUrl,
    };
}

export function buildSkillBadgeUrl(key: string, color: string) {
    return `https://cdn.simpleicons.org/${encodeURIComponent(key)}/${encodeURIComponent(color)}?viewbox=auto`;
}
