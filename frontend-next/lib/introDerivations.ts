  ...
  return [
    {
      value: 'CORE',
      label: '핵심 기술 스택',
      skills: coreSkills.filter((skill) => skill.usageType === 'WORK_EXPERIENCE').sort((a, b) => a.displayOrder - b.displayOrder),
    },
    {
...
export function buildMilestones(introData: IntroductionResponse): Milestone[] {
  return (introData.coreProjects ?? []).map((exp) => {
    ...
    const career = exp.careerId ? introData.experiences.find((item) => item.id === exp.careerId && item.type === 'CAREER') : undefined;
    ...
    return {
      ...
      role: career ? `${career.companyName || career.title} · ${exp.role || career.role || ''}` : (exp.role ?? ''),
...
export function buildOrderedCredentials(experiences: Experience[]): Experience[] {
  const creds = experiences.filter((exp) => exp.type === 'EDUCATION' || exp.type === 'CERTIFICATE');
