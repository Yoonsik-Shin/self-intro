import { Briefcase, Cpu, GraduationCap, MessageSquareText, Sparkles, User } from 'lucide-react';
...
export const printableSections: PrintSection[] = [
    ...
    { id: 'cover-letter', label: '사전질문', icon: MessageSquareText },
];

/** 프로필은 인쇄 프리뷰에서 제외/이동이 불가능하게 고정된다 */
export const LOCKED_PRINT_SECTION_ID = 'intro-profile';

export const reorderablePrintSections = printableSections.filter(
    (s) => s.id !== LOCKED_PRINT_SECTION_ID
);
