...
(9, 'azure-log-cost-retention-optimization', 'Azure 로그 비용 과다 진단 및 보관 기간 최적화 (RET-001, RET-002)',
 ...
 '# Azure 로그 비용 과다 진단 및 보관 기간 최적화 (RET-001, RET-002)\n\n## 1. 기술 배경 및 해결하고자 한 핵심 문제\n- 클라우드 환경에서 시스템 규모가 커짐에 따라 수집되는 로그 데이터 양이 급증하고, 이로 인해 Log Analytics Workspace(LAW) 청구 비용이 예측 불가능하게 늘어나는 ''빌링 쇼크(Billing Shock)''가 빈번히 발생했습니다.\n- 로그 요금 최적화를 위해서는 전체 수집을 막는 것이 아니라, 어떤 테이블에서 비용이 발생하는지(`Usage` 분석)와 보존 주기(Retention) 및 요금제(Plan)가 가치에 맞게 설정되었는지 진단하고 처방하는 지능형 모니터링이 필요했습니다.\n\n## 2. 기술 동작 원리 및 아키텍처 설계\n- **Usage 테이블 기반 실시간 비용 추적 (RET-001)**:\n  - `Usage` 시스템 테이블을 쿼리하여 실제 과금 대상인 테이블 (`IsBillable == true`)의 데이터 크기 (`Quantity` 및 `_BilledSize`)를 DataType별로 정렬 및 분석하는 KQL 수집 모듈 구현.\n- **Azure Retail Prices API 실시간 연동 & 계약 단가 차이 보완**:\n  - 리소스가 배포된 Azure 리전 속성을 식별하고, 해당 리전의 `Analytics Logs Data Ingestion` Pay-As-You-Go 단가를 동적으로 API 호출하여 정확한 비용을 USD로 환산.\n  - **Public Catalog vs EA 계약 단가 분리**: Retail Prices API는 공개 기본 단가를 제공하므로, Enterprise Agreement(EA) 또는 MCA 계약 고객의 할인가 오차를 방지하기 위해 Consumption API 실제 청구 단가 교차 검증 파이프라인을 구축.\n- **ARG(Azure Resource Graph) 기반 동적 예산 수집**:\n  - `workspaceCapping.dailyQuotaGb` 설정을 Resource Graph로 Bulk 조회하여 일일 예산 자동 계산.\n- **테이블 등급별 보존 주기 최적화 시뮬레이션 (RET-002)**:\n  - 로그 성격에 따른 분류 체계(Class A 보안/감사, Class B 운영/지표, Class C 추적/디버그) 정의.\n  - 테이블별 현재 보존일수와 사용량을 기준 31일 초과분에 대한 Archive 티어 전환 시 월간 예상 절감 비용 산출.\n\n## 3. 핵심 구현 및 트러블슈팅 인사이트\n- **사전 방어 파이프라인**: 일일 예산 대비 현재 소진율(`budget_ratio`)을 계산해 10% Warning, 25% Critical 경보를 발송하여 요금 폭탄을 사전 방지했습니다.\n- **컴플라이언스 양립**: 보안 로그 365일 규정 준수와 불필요한 디버그 로그의 Archive 티어 전환 처방을 동시에 달성했습니다.',
 'PUBLISHED', 5, '2026-06-25', '2026-06-25 10:00:00', NOW(), NOW()),

-- Study 10: Observability (LogDoctor & Dashboards)
(10, 'cloud-infrastructure-app-observability-diagnostics', '클라우드 인프라 생존 및 앱 관측성 진단 아키텍처와 Grafana 대시보드 해독 가이드',
 'App Service, Container Apps, VM 등 분산 인프라 텔레메트리 유입 분석 및 Prometheus & Grafana 10대 관측 대시보드 패널별 지표 해독과 SRE 장애 판별 임계치 수록.',
 '# 클라우드 인프라 생존 및 앱 관측성 진단 아키텍처 (DET-001 ~ DET-003)\n\n## 1. 기술 배경 및 해결하고자 한 핵심 문제\n- 분산 인프라 환경에서 리소스(VM, App Service 등)는 정상 가동 중이나 로그 에이전트가 중단되어 모니터링 사각지대가 발생하는 문제를 해결해야 했습니다.\n- 계정 정보나 Secret 키를 직접 수집하지 않는 보안 제약 하에, Log Analytics Workspace 텔레메트리만으로 관측 배관의 유효성과 앱 헬스 상태를 간접 판정하는 지능형 모듈을 설계했습니다.\n\n## 2. 기술 동작 원리 및 아키텍처 설계\n- **명시 연결 기반 Target Resolver (DET-001)**:\n  - Diagnostic Settings 및 hidden-link 메타데이터 등 명시적 연결 경로만 추적하여 신뢰도를 확보.\n  - App Service/ACA/ACI를 대상으로 `AppRequests`, `AppTraces` 유입 상태를 추적.\n- **VM 관측성 배관 진단 및 생존 분석 (DET-002)**:\n  - DCR Association 정보와 AMA(Azure Monitor Agent) 설치 여부를 교차 검증.\n  - `Heartbeat` 수신 상태와 Host Name을 기반으로 앱 후보군 매핑.\n- **HTTP 헬스 신호 및 응답 지연 탐지 & Sampling itemCount 가중치 반영 (DET-003)**:\n  - Adaptive Sampling 적용 시 단순히 average/percentile을 계산하면 표본 왜곡이 일어나므로 `sum(itemCount)` 표본 가중치 공식을 적용하여 정확한 HTTP 5xx 에러율 및 P95 Latency(`percentile(DurationMs, 95)`) 산출.\n\n## 3. 핵심 구현 및 트러블슈팅 인사이트\n- **배관 진단**: LAW 미연결 시 Critical, 플랫폼 로그만 유입 시 Warning, 관측성 완비 시 Healthy 단계별 진단 정착.\n- **장애 탐지**: 에러율 > 15% 또는 P95 Latency > 5,000ms 기반의 자동 경보 체계로 모니터링 대시보드 시각화 완수.\n\n## 4. 📊 Prometheus & Grafana 10대 관측 대시보드 해독 및 SRE 장애 판별 가이드\n\n시스템을 안정적으로 유지하고 장애 발생 시 즉각 대응하기 위한 **Grafana 10대 직관적 대시보드 해석 지침**입니다.\n\n### 1) `[App] Spring Boot API & Worker Metrics`\n- **JVM Heap Memory**: `Used`가 `Max`의 85% 이상 지속 상승 시 OOM(OutOfMemoryError) 임박. Full GC 발생 빈도 추적 후 Heap 스케일업 필요.\n- **HTTP Server Latency (P95/P99)**: P95 응답 속도가 2초를 초과하는 경우 external API 지연 또는 DB Lock 경합 장애 판정.\n- **HikariCP Connection Pool**: `Active Connections`가 `MaximumPoolSize`에 달하고 `Pending Threads > 0` 발생 시 DB 커넥션 고갈 장애.\n\n### 2) `[App Log] Central Log Viewer (Loki)`\n- **Error Log Rate Spike**: `level="ERROR"` 로그의 초당 생성 그래프가 튀는 경우 해당 스파이크 지점의 `traceId`를 복사하여 분산 트랜잭션 흐름 단입 추적.\n- **Log-to-Trace Correlation**: Loki 로그의 `[traceId=...]` 항목 클릭 시 해당 요청의 전체 마이크로서비스 호출 수명 주기 확인 가능.\n\n### 3) `[DB] MySQL Primary Database Health`\n- **Slow Queries**: `Slow Queries / sec > 0` 지속 발생 시 인덱스 미타 쿼리 탐지 및 실행 계획(EXPLAIN) 분석 실시.\n- **Buffer Pool Hit Ratio**: `InnoDB Buffer Pool Hit Ratio < 99%`일 경우 메모리 부족으로 디스크 I/O 병목 발생 조치 필요.\n\n### 4) `[DB] Oracle 26ai Vector Database Health`\n- **Active Sessions & Wait Class**: Active Sessions 수가 수치상 급증하고 `Wait Class = System I/O` 비율이 커지면 Vector Search 인덱스 재구성 검토.\n- **Tablespace Used %**: `Tablespace Used > 90%` 시 데이터 유실 방지를 위한 자동 스토리지 확장 조치.\n\n### 5) `[Queue] RabbitMQ Message Broker`\n- **Queued Messages (Queue Depth)**: `Ready Messages` 수치가 줄어들지 않고 누적되면 Consumer 처리 지연 또는 Worker Down 장애 상황.\n- **Unacknowledged Messages**: Consumer가 ACK를 반환하지 않고 잡고 있는 상태가 지속되면 Worker 스레드 데드락 판정.\n\n### 6) `[K8s] Workload Pod Health & Allocation`\n- **Container Restarts**: Restarts 횟수가 1시간 내 3회 이상 증가하면 CrashLoopBackOff 또는 OOMKilled 발생 증거.\n- **Memory Usage vs Limit**: Pod 메모리 사용량이 `Limit`의 90% 도달 시 K8s OOM Killer에 의해 파드 강제 종료 예고.\n\n### 7) `[Infra] Kubernetes Node Resources`\n- **Node CPU / RAM Pressure**: Node CPU Usage > 85%, RAM Usage > 90% 지속 시 워커 노드 스케일아웃(Autoscaling) 트리거 필요.\n\n### 8) `[Cache] Redis Memory & Session Health` (신규)\n- **Cache Hit Rate**: `Hit Rate < 80%` 하락 시 DB 직접 조회 트래픽이 폭증하므로 TTL 수명 및 캐시 워밍 정책 재검토.\n- **Evicted Keys**: `Evicted Keys > 0` 발생 시 `maxmemory` 한계 도달로 세션 쿠키 탈락 리스크 경보.\n\n### 9) `[Gateway] Ingress NGINX Traffic & Latency` (신규)\n- **4xx vs 5xx Error Rate**: 4xx 급증은 악성 스크래핑/DDoS 요청 탐지, 5xx 급증은 백엔드 Pod 장애 또는 NGINX Gateway Timeout 상황.\n\n### 10) `[Business] AI Worker & Scraper Pipeline Ops` (신규)\n- **Scrape Success %**: `Success Rate < 95%` 하락 시 대상 채용 사이트(사람인/잡코리아) DOM 변경 또는 Captcha 차단 유발 탐지.\n- **OpenAI Latency & Tokens**: OpenAI 평균 응답 시간이 5초를 넘거나 토큰 소비량이 급증하면 프롬프트 최적화 및 쿼터 점검.',
...
(11, 'intelligent-log-filtering-pii-masking-engine', '지능형 로그 필터링 및 민감 정보 마스킹 엔진 (PRV-001, PRV-002, PRV-003, FLT-001, FLT-002, FLT-003)',
 '운영 환경의 상세 로그 활성화 여부 진단, 수집 시점(Ingestion-time) 정규식을 통한 개인정보(PII) 유출 탐지/마스킹, 에러 로그 컨텍스트 3요소 품질 평가 점수화, 고빈도 노이즈 분석을 통한 DCR 필터링 자동 처방 구현.',
 '# 지능형 로그 필터링 및 민감 정보 마스킹 엔진 (PRV-001, PRV-002, PRV-003, FLT-001, FLT-002, FLT-003)\n\n## 1. 기술 배경 및 해결하고자 한 핵심 문제\n- 개발 단계의 디버그 로그가 프로덕션에 방치되어 불필요한 청구 비용이 발생하거나, 사용자 계정 정보(비밀번호, 토큰 등)가 평문 수집되어 컴플라이언스를 위반하는 리스크를 방지하고자 했습니다.\n- 무의미한 노이즈 로그의 유입을 막고, 에러 로그의 3대 필수 컨텍스트(Cause, Location, Action) 보유 여부를 정량적으로 점수화하는 엔진을 구현했습니다.\n\n## 2. 기술 동작 원리 및 아키텍처 설계\n- **운영 환경 디버그 로그 방치 감지 (PRV-001)**:\n  - App Settings 환경변수와 KQL `SeverityLevel <= 1` 유입 상태를 교차 검증해 하드코딩된 디버그 로거 판별.\n- **Ingestion-time(수집 시점) PII 개인정보 마스킹 (FLT-001)**:\n  - LAW 저장 전 수집기(DCR Transformation KQL / Collector Processor) 단계에서 이메일, 전화번호, API Token 정규식 패턴을 즉시 마스킹.\n  - **컴플라이언스 준수 원칙**: LAW 조회 시점 마스킹이 아닌 Ingestion 시점 마스킹을 적용해 원본 LAW 디스크 스토어에 평문 PII가 저장되는 컴플라이언스(ISMS-P, GDPR) 위반 리스크를 근본 차단.\n- **고빈도 노이즈 지문 분석 및 LLM KQL AST 검증 (PRV-002, FLT-003)**:\n  - 앞 150글자 지문(Fingerprint) 기반 상위 노이즈 패턴 추출 및 LLM 활용 DCR Transformation KQL 생성.\n  - **KQL AST Validator 도입**: LLM이 생성한 KQL을 파이프라인에 즉시 배포하지 않고, 문법 검증기(AST Validator) 및 Dry-run을 거쳐 정상 로그 수집 중단 리스크 예방.\n- **에러 로그 컨텍스트 품질 점수화 (FLT-002)**:\n  - Cause(예외 타입), Location(스택트레이스), Action(조치 힌트) 3요소 품질 평가 점수화.\n\n## 3. 핵심 구현 및 트러블슈팅 인사이트\n- **보안 & 비용 동시 확보**: Ingestion-time PII 마스킹을 통한 컴플라이언스 완벽 준수와 노이즈 로그 필터링을 통한 비용 절감을 동시에 달성했습니다.\n- **디버깅 SNR 최적화**: 에러 로그 3요소 평가를 통해 실무 개발팀의 장애 조치 리드타임을 대폭 감소시켰습니다.',
