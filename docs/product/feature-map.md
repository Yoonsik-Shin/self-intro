...
| --------------------------- | ---------------------------- | -------------------------------- | ------------ |
| 로그인 이메일·비밀번호      | Account 본인                 | 비밀번호 변경·분실 재설정 구현, 이메일 변경 미구현 | 일부 구현    |
...
- 계정 설정에서 닉네임 변경, 가입과 동일한 10~32자 비밀번호 정책을 적용한 현재 비밀번호 기반 변경,
  ...
  재설정은 30분 hash token, 일회성 소비, 발송 실패 회수와 전체 세션 폐기로 구현했다. 이메일 변경은
  새 주소 확인·충돌 회수 정책이 필요해 별도 운영 차단 항목으로 남는다.
