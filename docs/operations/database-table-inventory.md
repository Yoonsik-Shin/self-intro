...
- 로컬 기준: Flyway V231, 애플리케이션 테이블 112개 + `flyway_schema_history` 1개
...
- 현재 113개 테이블은 아래 8개 책임 영역에 중복·누락 없이 분류된다.
...
`career`, `certificate`, `competency`, `competency_evidence`, `competency_skill`, `competency_study`,
...
`study_plan`, `study_plan_candidate`, `study_plan_check_question`, `study_plan_item`, `study_plan_message`,
`study_plan_stage`, `study_relation`, `study_skill`, `study_tag`, `study_taxonomy_curation`,
`study_taxonomy_node`, `taxonomy_node`, `taxonomy_scheme`, `workspace_learning_resource`,
`workspace_learning_resource_tag`

Markdown 학습 원본, 학습 자료, AI 학습 계획과 도메인 taxonomy를 포함한다. `study_plan_*`와
`learning_resource_*`는 현재 backend·AI 경로에서 사용하므로 초기 데이터가 없어도 유지한다.

### 공개 구성·revision·출력 (16)

`print_template`, `print_template_revision`, `workspace_public_competency_selection`,
...
selection, revision, publication은 수명주기가 다르므로 하나로 합치지 않는다.
...
`workspace_job_application`, `workspace_job_application_status_event`, `workspace_job_screenshot_upload`

공고 원본·출처와 Workspace별 지원 상태를 분리한다. `job_posting`을 Workspace의 지원 메모와 합치면
공통 원본과 사적 상태의 소유권이 섞이므로 유지한다. `gap_project_document`도 활성 API 경로가 있다.
...
3. 공개 revision과 브라우저 PDF를 불변 산출물로 연결하는 작업에서는
   ...
   먼저 재검증한다.
4. 운영 반영 전에는 로컬 113개와 운영 schema를 직접 비교하고, V231 이전 운영 환경에 DROP을
   선적용하지 않는다.
