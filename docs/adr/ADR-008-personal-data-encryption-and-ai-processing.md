...
바꾸거나 BYOK를 사용하더라도 플랫폼이 어떤 정보를 선택해 전송했고 누가 동의했는지에 대한 책임은
남는다.
...
| OWNER MFA | 일반 사용 전체에 일괄 강제하지 않고 BYOK 등록·교체·폐기, 결제수단 변경, 소유권 이전, 멤버 권한 변경, 전체 내보내기와 Workspace 삭제에 step-up MFA를 의무화한다. 조직·Business Workspace OWNER는 해당 기능을 활성화하기 전에 MFA를 등록해야 한다. |
...
| `SECRET` | 비밀번호 검증값, MFA secret, 복구 코드, 세션·초대 token, PG·Provider·BYOK key | 단방향 검증 또는 Secret Manager, 원문 조회 API 금지 |
...
| PG·플랫폼 Provider·BYOK key | 애플리케이션 컬럼에 평문·복호화 가능 암호문으로 보관하지 않고 Secret Manager reference만 저장 |
...
Workspace `OWNER`는 허용 Provider, 모델 등급, 처리 region, BYOK 사용, 멤버별 예산과 데이터 전송
...
- 작업 목적과 결과물
- 플랫폼 관리 key, Workspace BYOK 또는 local/self-hosted 중 실제 처리 경로
- Provider, 모델, 처리 region·국가와 공개된 보존 정책
- 전달할 데이터 범주와 제외되는 데이터
- 예상 point 범위와 BYOK의 고객 직접 청구 여부
...
Provider, region, 플랫폼/BYOK 경로, 전송 범주 또는 정책의 중요한 내용이 바뀌면 OWNER 정책 확인과
...
`ai_usage`에는 Provider, model, region, BYOK 여부, token, retry, 원가 snapshot, 제한된 결과 코드,
...
- BYOK 실패·quota 초과·embedding 장애 시 플랫폼 key나 다른 Provider로 자동 fallback하지 않는다.
...
### 8. BYOK와 결제 Secret은 서비스별 IAM으로 분리한다
...
| AI Worker | 승인된 요청의 AI Provider/BYOK key를 호출 시점에 사용 | PG secret·billing key 접근 |
| Billing Worker | PG secret·billing key를 결제 작업에 사용 | AI Provider/BYOK key 접근 |
...
- 소유권 이전 시 결제수단과 BYOK를 중지하고 새 OWNER의 재확인 전 자동 승계하지 않는다.
...
- BYOK embedding 실패 시 플랫폼 embedding으로 자동 전환하지 않는다.
...
- Public API 배포에는 Private DB, Vector DB, AI Provider, BYOK와 PG Secret을 주입하지 않는다.
...
7. SecretProvider port, AI/Billing Worker IAM 분리와 BYOK key 등록·회전·폐기를 구현한다.
...
다음 조건을 모두 통과하기 전에는 BYOK, 유료 AI point와 강화된 개인정보 처리를 운영 flag로 열지 않는다.
...
- BYOK 실패·quota 초과·embedding 장애의 no-fallback 검증
...
- 플랫폼 key, BYOK와 local model이 동일한 권한·동의·감사·삭제 계약을 사용한다.
...
- BYOK는 Provider별 오류·region·model·embedding 계약과 사용자 지원 범위를 늘린다.
...
- BYOK key의 평문 또는 애플리케이션 DB 저장: 유출 반경과 원문 조회 가능성을 키우므로 금지한다.
- BYOK 실패 시 플랫폼 key 자동 fallback: 예상하지 않은 비용·데이터 처리 경로를 만들므로 금지한다.
