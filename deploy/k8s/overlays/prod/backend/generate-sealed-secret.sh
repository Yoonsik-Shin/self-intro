#!/bin/bash
# 이 스크립트는 OKE 클러스터의 backend-db-secret을 가져와 SealedSecret으로 암호화합니다.

NAMESPACE="self-intro"
SECRET_NAME="backend-db-secret"
OUTPUT_FILE="deploy/k8s/overlays/prod/backend/sealed-db-secret.yaml"
...
echo "  git commit -m \"deploy: add sealed db secret\""
echo "  git push"
