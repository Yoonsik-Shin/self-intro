...
class CareerProfileDigestBuilderTest {
    ...
    @BeforeEach
    void setUp() {
        builder = new CareerProfileDigestBuilder(experienceRepository, competencyRepository, studyRepository);
    }

    @Test
    void buildsDigestIncludingExperienceCompetencyAndStudy() {
        Project experience = Project.create("프로젝트 A", LocalDate.now(), null, "설명", "배운점", 0, List.of(), List.of(), "project-a", "개발자", 100);
        Competency competency = Competency.create("백엔드 설계", "요약", 0, true);
        Study study = Study.create("oracle-study", "Oracle 26ai 학습", "Vector DB 및 SQL 공부", "내용", StudyStatus.PUBLISHED, LocalDate.now(), null);

        when(experienceRepository.findAllByOrderByDisplayOrderAsc()).thenReturn(List.of(experience));
        when(competencyRepository.findAllByVisibleTrueOrderByDisplayOrderAsc()).thenReturn(List.of(competency));
