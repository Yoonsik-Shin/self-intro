...
/**
 * 콘텐츠(내용) 조회 전용 공개 API. 인쇄/PDF 배치(PrintTemplate, document_type=PORTFOLIO)는 관리자 전용이라 여기에는 없다 — 관리자
 * 화면(포트폴리오 관리 → PDF 템플릿 관리)에서만 다룬다.
 */
@RestController
...
public class PortfolioCaseStudyPublicController {
    ...
    public ResponseEntity<List<PortfolioCaseStudyPublicSummaryResponse>> list() {
        return ResponseEntity.ok(portfolioCaseStudyService.listPublished(workspaceId()));
    }

    @GetMapping("/{slug}")
    public ResponseEntity<PortfolioCaseStudyPublicResponse> get(@PathVariable String slug) {
        return ResponseEntity.ok(portfolioCaseStudyService.getPublishedBySlug(workspaceId(), slug));
    }
    ...
    public ResponseEntity<List<PortfolioCaseStudyPublicSummaryResponse>> listByStudy(
        ...
        return ResponseEntity.ok(
                portfolioCaseStudyService.listPublishedByStudyId(workspaceId(), studyId));
    }
}
