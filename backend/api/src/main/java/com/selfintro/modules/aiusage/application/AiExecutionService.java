...
public class AiExecutionService {
    ...
    public <T> T execute(AiExecutionCommand command, Supplier<T> providerCall) {
        ...
        boolean previewAllowed =
                previewPolicy.isAllowed(command.actorUserId(), command.workspaceId());
        if (!generationEnabled && !previewAllowed) {
            throw new ResponseStatusException(
                    HttpStatus.SERVICE_UNAVAILABLE, "비공개 베타에서는 외부 AI 기능을 제공하지 않습니다.");
        }

        boolean enforceUsage = generationEnabled || previewAllowed;
        AiProcessingRoute route = consentService.requireOrRecord(command, enforceUsage);
