-- V83: Add 3 Predicted Hyundai Autoever Mock Problems (Draft Status: DRAFT)
...
-- =========================================================================
-- 1. 현대오토에버 대비 모의문제 5 — 스마트 팩토리 로봇 수송 안전거리 (DRAFT)
-- =========================================================================
INSERT INTO study (
    slug, title, summary, content_markdown, status, category_id, learned_at, published_at, created_at, updated_at
) VALUES (
    'autoever-mock-05-smart-factory-robot-routing',
    '현대오토에버 대비 모의문제 5 — 스마트 팩토리 로봇 수송 안전거리',
    '[초안 / 출제 예측 모의문제 5] 현대오토에버 스마트 팩토리 무인 운반차(AGV) 수송 시스템에서 충돌 없이 운행하기 위한 안전거리 최댓값을 구하는 출제 예측 모의문제다. 매개 변수 탐색과 BFS를 결합해 해결한다.',
    '# 현대오토에버 대비 모의문제 5 — 스마트 팩토리 로봇 수송 안전거리

> [초안 - DRAFT] 현대오토에버 스마트 팩토리(E-FOREST) 물류 관제 출제 경향을 대비하기 위한 모의문제 해설 초안 노트입니다.
...
<details>
<summary>▶ [정답 보기] 풀이 방법 및 프로그래머스 규격 Java 완성 코드</summary>
...
',
    'DRAFT', @education_category_id, '2026-07-25', NOW(), NOW(), NOW()
) ON DUPLICATE KEY UPDATE title=VALUES(title), summary=VALUES(summary), content_markdown=VALUES(content_markdown), status='DRAFT', updated_at=NOW();


-- =========================================================================
-- 2. 현대오토에버 대비 모의문제 6 — 차량 OTA 롤백 최적화 (DRAFT)
-- =========================================================================
INSERT INTO study (
    slug, title, summary, content_markdown, status, category_id, learned_at, published_at, created_at, updated_at
) VALUES (
    'autoever-mock-06-ota-subsystem-rollback',
    '현대오토에버 대비 모의문제 6 — 차량 OTA 서브시스템 롤백 최적화',
    '[초안 / 출제 예측 모의문제 6] 무선 소프트웨어 업데이트(OTA) 중 통신 세션 단절 시 제어기 모듈 간 의존 관계를 보존하며 안정 상태로 롤백하는 출제 예측 모의문제다. 유니온 파인드와 위상 정렬을 결합해 해결한다.',
    '# 현대오토에버 대비 모의문제 6 — 차량 OTA 서브시스템 롤백 최적화

> [초안 - DRAFT] 현대오토에버 차량 제어기 무선 업데이트(OTA, Over-The-Air) 관제 시스템 출제 경향을 대비하기 위한 모의문제 해설 초안 노트입니다.
...
-- =========================================================================
-- 3. 현대오토에버 대비 모의문제 7 — 라이다 센서 밀도 구간 필터링 (DRAFT)
-- =========================================================================
INSERT INTO study (
    slug, title, summary, content_markdown, status, category_id, learned_at, published_at, created_at, updated_at
) VALUES (
    'autoever-mock-07-lidar-sensor-segment-collection',
    '현대오토에버 대비 모의문제 7 — 라이다 센서 밀도 구간 필터링',
    '[초안 / 출제 예측 모의문제 7] 자율주행 시험로 격자에서 라이다(LiDAR) 포인트 밀도가 임계값 이상인 최대 정사각형 서브 영역을 탐색하는 출제 예측 모의문제다. 2차원 누적 합과 이분 탐색을 결합해 해결한다.',
    '# 현대오토에버 대비 모의문제 7 — 라이다 센서 밀도 구간 필터링

> [초안 - DRAFT] 현대오토에버 자율주행 라이다(LiDAR) 데이터 수집 및 구간 필터링 출제 경향을 대비하기 위한 모의문제 해설 초안 노트입니다.
...
-- =========================================================================
-- Tag & Skill Mappings for Predicted Draft Mock Problems
-- =========================================================================
INSERT INTO study_tag (study_id, tag_id)
SELECT s.id, t.id
FROM (
    SELECT 'autoever-mock-05-smart-factory-robot-routing' AS study_slug, '현대오토에버' AS tag_name UNION ALL
    SELECT 'autoever-mock-05-smart-factory-robot-routing', '모의문제' UNION ALL
    SELECT 'autoever-mock-05-smart-factory-robot-routing', '이분 탐색' UNION ALL
    SELECT 'autoever-mock-05-smart-factory-robot-routing', 'BFS' UNION ALL
    SELECT 'autoever-mock-05-smart-factory-robot-routing', 'Java' UNION ALL

    SELECT 'autoever-mock-06-ota-subsystem-rollback', '현대오토에버' UNION ALL
    SELECT 'autoever-mock-06-ota-subsystem-rollback', '모의문제' UNION ALL
    SELECT 'autoever-mock-06-ota-subsystem-rollback', '유니온 파인드' UNION ALL
    SELECT 'autoever-mock-06-ota-subsystem-rollback', '위상정렬' UNION ALL
    SELECT 'autoever-mock-06-ota-subsystem-rollback', 'Java' UNION ALL

    SELECT 'autoever-mock-07-lidar-sensor-segment-collection', '현대오토에버' UNION ALL
    SELECT 'autoever-mock-07-lidar-sensor-segment-collection', '모의문제' UNION ALL
    SELECT 'autoever-mock-07-lidar-sensor-segment-collection', '누적 합' UNION ALL
    SELECT 'autoever-mock-07-lidar-sensor-segment-collection', '이분 탐색' UNION ALL
    SELECT 'autoever-mock-07-lidar-sensor-segment-collection', 'Java'
) mapping
