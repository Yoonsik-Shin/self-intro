-- V72: Force update study content for autoever-mock-01 and bitmask-dp-dijkstra-algorithm to apply details toggle and pure independent algorithm note

SET NAMES utf8mb4 COLLATE utf8mb4_unicode_ci;

-- 1. Update autoever-mock-01-required-checkpoints with details toggle solution
UPDATE study
SET title = '현대오토에버 대비 모의문제 1 — 필수 점검소를 경유하는 테스트카',
    summary = '필수 점검소를 모두 방문하는 최소 운행 시간을 구하는 현대오토에버 모의문제 풀이 노트다. 정답 및 풀이 보기 토글 내에 라인별 상세 한글 주석이 포함된 자바 정답 코드를 제공한다.',
    content_markdown = '# 현대오토에버 대비 모의문제 1 — 필수 점검소를 경유하는 테스트카

> 공개된 현대오토에버 코딩테스트 후기에서 출제되는 조건부 최단 경로와 DP 결합 경향을 대비하기 위한 모의문제 해설 노트입니다.
...
이 문제는 **다익스트라(Dijkstra)** 최단 경로 알고리즘과 **비트마스크 동적 계획법(Bitmask DP)**을 결합하여 해결합니다.

1. **중요 지점 간 거리 행렬 구축 (Dijkstra)**:
   - 전체 정점 수 $N=100,000$ 그래프에서 모든 지점 경로를 직접 비트마스크 탐색하는 것은 불가능합니다.
   - 출발점(`1`), 필수 점검소 `K`개, 도착점(`N`) 등 총 $(K+2)$개 중요 지점에서 다익스트라 알고리즘을 수행하여 중요 지점 간 최단 거리 테이블 `between[][]`을 구축합니다.
2. **방문 순서 최적화 (Bitmask DP)**:
   - $K \\le 8$로 매우 작으므로, 점검소 방문 여부를 비트마스크(`mask`) 정수로 표현합니다.
   - `dp[mask][last]` : 현재 방문한 점검소 비트 집합이 `mask`이고 마지막 위치가 `last`번째 점검소일 때의 최소 소요 시간을 점화식으로 갱신합니다.
3. **최종 정답 산출**:
   - 모든 점검소를 방문한 상태(`fullMask`)에서 최종 `N`번 지점으로 이동하는 최솟값을 구합니다.
...
### 복잡도 분석
- **시간복잡도**: $O((K + 2)(N + M) \\log N + 2^K \\cdot K^2)$ ($N=100,000, M=200,000, K=8$ 시 약 0.08초 소요)
...
이 문제의 기반이 되는 일반화된 알고리즘 이론은 **[비트마스크 DP & 다익스트라 알고리즘 정리](http://localhost:3000/study/bitmask-dp-dijkstra-algorithm)** 스터디 노트에서 학습할 수 있습니다.
',
    status = 'PUBLISHED',
    updated_at = NOW()
WHERE slug = 'autoever-mock-01-required-checkpoints';

-- 2. Update bitmask-dp-dijkstra-algorithm with 100% independent algorithm theory
UPDATE study
SET title = '비트마스크 DP & 다익스트라 (Bitmask DP & Dijkstra) 알고리즘 정리',
    summary = '그래프의 특정 서브셋 정점(Subset Vertices)들을 모두 거쳐 목적지까지 이동하는 최단 경로를 구하는 대표적인 결합 기법이다. 대규모 그래프를 주요 경유 정점만으로 추상화(Dijkstra)한 뒤 비트마스크 정수 상태와 동적 계획법(Bitmask DP / TSP)으로 경로를 최적화한다.',
    content_markdown = '# 비트마스크 DP & 다익스트라 (Bitmask DP & Dijkstra) 알고리즘 정리
...
',
    status = 'PUBLISHED',
    updated_at = NOW()
WHERE slug = 'bitmask-dp-dijkstra-algorithm';
