...
/**
 * IP당 분당 5회 후원 생성 제한. 단일 replica 배포라 인메모리로 충분하다 (replica가 늘어나면 제한이 replica 수만큼 느슨해지는 한계가 있음). 큐 갱신은
 * 반드시 compute 계열 블록 안에서만 수행한다 — ConcurrentHashMap의 키 단위 원자성이 동시성 보장의 전부이고, ArrayDeque 자체는
 * thread-safe가 아니다.
 */
@Component
...
public class DonationRateLimiter {
    ...
    private final ConcurrentHashMap<String, ArrayDeque<Instant>> requestLog =
