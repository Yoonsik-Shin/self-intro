...
public class ExperiencePlacementService {
    ...
    public List<ExperiencePlacementResponse> getAll(ExperiencePlacementType placementType) {
        return placementRepository
                .findAllByPlacementTypeOrderByDisplayOrderAsc(placementType)
                .stream()
                .map(this::toResponse)
                .toList();
    }
    ...
    public List<ExperiencePlacementResponse> replaceAll(
            ExperiencePlacementType placementType, List<ExperiencePlacementRequest> requests) {
        ...
        Map<Long, Experience> experiences =
                experienceRepository.findAllById(uniqueIds).stream()
        ...
        Map<Long, ExperiencePlacement> placementsByExperienceId =
                saved.stream()
                        .collect(
                                Collectors.toMap(
                ...
                safeRequests.stream()
                        .flatMap(
                                request -> {
                                    ...
                                    Map<Long, ExperienceDetail> detailsById =
    ...
    private List<Long> validateAndNormalizeDetailIds(
            Experience experience, List<Long> requestedDetailIds) {
        List<Long> availableDetailIds =
                experience.getDetails().stream().map(ExperienceDetail::getId).toList();
        List<Long> selectedIds =
                requestedDetailIds == null ? availableDetailIds : requestedDetailIds;
    ...
    private List<Long> getSelectedDetailIds(ExperiencePlacement placement) {
        return placementDetailRepository
                .findAllByPlacementIdOrderByDisplayOrderAsc(placement.getId())
                .stream()
