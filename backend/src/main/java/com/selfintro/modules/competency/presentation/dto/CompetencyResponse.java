...
public record CompetencyResponse(
    ...
    public static CompetencyResponse from(Competency competency, boolean publicOnly) {
        List<EvidenceResponse> evidences = competency.getEvidences().stream()
            .sorted(Comparator.comparing(CompetencyEvidence::isPrimary).reversed()
                .thenComparingInt(CompetencyEvidence::getDisplayOrder))
            .map(EvidenceResponse::from)
            .toList();
        List<StudyReferenceResponse> studies = competency.getStudyLinks().stream()
        ...
        return new CompetencyResponse(
            competency.getId(), competency.getTitle(), competency.getSummary(), competency.getDisplayOrder(),
            competency.isVisible(),
            competency.getSkillLinks().stream().map(link -> SkillResponse.from(link.getSkill())).toList(),
            evidences, studies);
    }
    ...
    public record EvidenceResponse(
        ...
        static EvidenceResponse from(CompetencyEvidence evidence) {
            ...
            if (summary == null || summary.isBlank()) {
                summary = experience.getTakeaway() != null ? experience.getTakeaway() : experience.getSummary();
            }
            return new EvidenceResponse(evidence.getId(), experience.getId(), experience.getType(),
                experience.getTitle(), summary, evidence.isPrimary(), evidence.getDisplayOrder());
        }
    }
    ...
    public record StudyReferenceResponse(Long id, String slug, String title, String status) {
        static StudyReferenceResponse from(Study study) {
            return new StudyReferenceResponse(study.getId(), study.getSlug(), study.getTitle(), study.getStatus().name());
        }
    }
}
