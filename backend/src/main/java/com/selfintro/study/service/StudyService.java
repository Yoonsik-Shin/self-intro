...
public class StudyService {
    ...
    public StudyPageResponse searchPublished(String keyword, String category, List<String> tags,
                                             ...
                                             int page, int size) {
        return search(keyword, category, tags, skillIds, experienceIds, experienceDetailIds, StudyStatus.PUBLISHED, page, size);
    }

    public StudyPageResponse searchAdmin(String keyword, String category, List<String> tags,
                                         List<Long> skillIds, List<Long> experienceIds,
                                         List<Long> experienceDetailIds,
                                         StudyStatus status, int page, int size) {
        return search(keyword, category, tags, skillIds, experienceIds, experienceDetailIds, status, page, size);
    }

    private StudyPageResponse search(String keyword, String category, List<String> tags,
                                     List<Long> skillIds, List<Long> experienceIds,
                                     List<Long> experienceDetailIds,
                                     StudyStatus status, int page, int size) {
    ...
    public StudyResponse create(StudyRequest request) {
        ...
        LocalDateTime publishedAt = resolvePublishedAt(request.status(), request.publishedAt(), null);
    ...
    public StudyResponse update(Long id, StudyRequest request) {
        ...
        LocalDateTime publishedAt = resolvePublishedAt(request.status(), request.publishedAt(), study.getPublishedAt());
    ...
    private List<String> applyAssociations(Study study, StudyRequest request) {
        ...
        List<Experience> experiences = request.experienceIds() == null
                ? List.of() : experienceRepository.findAllById(request.experienceIds());
        List<ExperienceDetail> experienceDetails = request.experienceDetailIds() == null
                ? List.of() : experienceDetailRepository.findAllById(request.experienceDetailIds());
    ...
    private void applyRelations(Study source, List<StudyRelationRequest> requests) {
        ...
        for (int i = 0; i < requests.size(); i++) {
            ...
            Study target = studyRepository.findById(request.studyId())
                    .orElseThrow(() -> new EntityNotFoundException("Related study not found: " + request.studyId()));
    ...
    private String uniqueSlug(String requested, String title, Long currentId) {
        ...
        int suffix = 2;
        while (currentId == null ? studyRepository.existsBySlug(candidate)
    ...
    private LocalDateTime resolvePublishedAt(StudyStatus status, LocalDateTime requested, LocalDateTime existing) {
