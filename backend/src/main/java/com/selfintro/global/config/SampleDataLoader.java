...
public class SampleDataLoader implements ApplicationRunner {
    ...
    private void syncExistingStudies() {
        ...
        if (studyRepository
                .findBySlug("spring-security-session-redis-distributed-auth-architecture")
                .isEmpty()) {
            StudyCategory backendCategory =
                    studyCategoryRepository.findAll().stream()
                            .filter(
                                    c ->
                                            "backend".equalsIgnoreCase(c.getSlug())
                                                    || "backend".equalsIgnoreCase(c.getName()))
                            .findFirst()
                            .orElse(null);
            Study study5 =
                    studyRepository.save(
                            Study.create(
                                    ...
                                    LocalDateTime.of(2026, 7, 24, 23, 0)));
            if (!allDetails.isEmpty()) {
                study5.replaceExperienceDetails(List.of(allDetails.get(0)));
            }
            Map<String, Skill> skillMap = new HashMap<>();
            skillRepository.findAll().forEach(s -> skillMap.put(s.getName(), s));
            study5.replaceSkills(
                    getSkills(
                            List.of(
                                    "Java",
