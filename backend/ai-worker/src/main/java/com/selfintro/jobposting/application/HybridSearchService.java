...
/**
 * 하이브리드 검색 (Hybrid Search) & Reciprocal Rank Fusion (RRF) 랭킹 엔진 1) Sparse Lexical Keyword Matching
 * (MySQL 8.0 / Exact Match) 2) Dense Semantic Vector Search (Oracle 26ai Native VECTOR_DISTANCE) 3)
 * RRF (0.7 * Dense + 0.3 * Lexical) Combined Score Ranking
 */
@Slf4j
@Service
...
public class HybridSearchService {
    ...
    private final ProfileVectorSearchPort profileVectorSearchPort;
    ...
    public List<HybridMatchResult<ExperienceMatch>> searchTopSimilarExperiences(
            Long workspaceId, String queryText, List<String> requiredKeywords, int limit) {
        String queryVector = vectorEmbeddingService.embedToVectorString(queryText);
        List<ExperienceMatch> vectorResults =
                profileVectorSearchPort.findTopSimilarExperienceChunks(
                        workspaceId, queryVector, limit * 2);

        List<HybridMatchResult<ExperienceMatch>> hybridResults = new ArrayList<>();
        for (ExperienceMatch expVec : vectorResults) {
            double lexicalScore = calculateLexicalScore(expVec.chunkContent(), requiredKeywords);
            // Oracle 26ai COSINE VECTOR_DISTANCE(0=동일 ~ 2=반대) -> 유사도(1.0 - distance)
            double vectorScore = 1.0 - expVec.distance();
            double hybridScore = 0.7 * vectorScore + 0.3 * lexicalScore;

            hybridResults.add(
                    new HybridMatchResult<>(
                            expVec, expVec.chunkContent(), hybridScore, vectorScore, lexicalScore));
        }
    ...
    public List<HybridMatchResult<StudyVectorMatch>> searchTopSimilarStudies(
            Long workspaceId, String queryText, List<String> keywords, int limit) {
        String queryVector = vectorEmbeddingService.embedToVectorString(queryText);
        List<StudyVectorMatch> vectorResults =
                studyVectorRepository.findTopSimilarStudyChunks(
                        workspaceId, queryVector, limit * 2);
