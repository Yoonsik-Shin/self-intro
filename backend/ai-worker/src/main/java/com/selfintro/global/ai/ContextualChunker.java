...
/**
 * 2026 SOTA RAG Chunking Engine 1) Recursive Character Splitting (512 Tokens / 20% Overlap) 2)
 * Contextual Retrieval Header Injection (Anthropic SOTA - 검색 실패율 67% 감축)
 */
@Component
public class ContextualChunker {
     ...
     * @param contextualHeader 청크 헤더로 결합할 글로벌 문맥 메타데이터 (예: "[프로젝트: Self-Intro | 기술스택: Java, Spring,
     *     Oracle 26ai]")
    ...
    public List<String> chunkTextWithContext(String rawText, String contextualHeader) {
        ...
        List<String> rawChunks =
                splitRecursively(cleanedText, DEFAULT_CHUNK_SIZE_CHARS, DEFAULT_OVERLAP_CHARS);
        List<String> contextualizedChunks = new ArrayList<>();

        String headerPrefix =
                (contextualHeader != null && !contextualHeader.isBlank())
