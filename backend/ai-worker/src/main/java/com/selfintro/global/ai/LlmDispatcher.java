...
public class LlmDispatcher {
    ...
    public String generate(String systemPrompt, String userPrompt, String aiModel, String customModelName) {
        ...
        return switch (modelKey) {
            ...
            case "CUSTOM" -> dispatchCustom(systemPrompt, userPrompt, customModelName, false, 0, null);
            default -> nvidiaNimClient.generate(systemPrompt, userPrompt);
        };
    }

    /**
     * JSON 강제 응답 생성 — GapProject/PrintDraft처럼 구조화 응답 파싱이 필요한 곳에서 사용. Claude/Gemini는 네이티브 강제모드가
     * 없어 시스템 프롬프트 지시문(ClaudeAiClient) 또는 responseMimeType(GeminiAiClient)으로 흉내낸다.
     */
    public String generateJson(
        ...
        return switch (modelKey) {
            ...
            case "GEMINI_3_6_FLASH" -> geminiAiClient.generateJson(systemPrompt, userPrompt, "gemini-3.6-flash");
            case "GEMINI_3_1_FLASH_LITE", "GEMINI" ->
                    geminiAiClient.generateJson(systemPrompt, userPrompt, "gemini-3.1-flash-lite");
            case "GPT_5_4_NANO" -> openAiClient.generateJson(systemPrompt, userPrompt, "gpt-5.4-nano");
            case "GPT_5_4_MINI", "GPT_4O", "GPT" -> openAiClient.generateJson(systemPrompt, userPrompt, "gpt-5.4-mini");
            case "CUSTOM" -> dispatchCustom(systemPrompt, userPrompt, customModelName, true, maxOutputTokens, timeout);
            default -> nvidiaNimClient.generateJsonOnce(systemPrompt, userPrompt, maxOutputTokens, timeout);
        };
    }
