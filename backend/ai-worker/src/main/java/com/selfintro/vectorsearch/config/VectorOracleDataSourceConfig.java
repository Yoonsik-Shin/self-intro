...
/**
 * job_posting_vector/experience_vector/study_vector 3개 테이블은 Oracle 26ai 전용이라 기본(1차) 데이터소스 (MySQL,
 * {@code PrimaryDataSourceConfig})로는 접근할 수 없다 — 별도 2차 데이터소스로 분리 연결한다.
 *
 * <p>이 클래스가 {@code com.selfintro.vectorsearch} 패키지 안에 있는 건 의도적이다 — api의 {@code
 * SelfIntroApplication}이 이 패키지 전체를 @ComponentScan에서 제외하므로, api pod는 이 설정도 Oracle 접속도 아예 필요
 * 없다(2026-08 job-posting CRUD/AI 분리 이후 api는 벡터 검색을 쓰지 않는다). worker만 이 클래스를 스캔해서 vectorsearch 리포지토리를
 * 2차 데이터소스로 연결한다.
 */
@Configuration
...
public class VectorOracleDataSourceConfig {
    ...
    public PlatformTransactionManager vectorTransactionManager(
            @Qualifier("vectorEntityManagerFactory")
                    EntityManagerFactory vectorEntityManagerFactory) {
