...
import type { Competency } from '../lib/api';
      ...
      <div className="grid gap-8 px-5 py-6 sm:px-7 lg:grid-cols-[minmax(0,1fr)_260px] lg:py-8">
        ...
        <aside className="space-y-5 lg:border-l lg:border-slate-100 lg:pl-6">
          <section>
            ...
            {competency.skills.length > 0 ? (
              <div className="flex flex-wrap gap-1.5">
                {competency.skills.map((skill) => <span key={skill.id} className="rounded-md bg-blue-50 px-2 py-1 text-xs font-bold text-blue-700">{skill.name}</span>)}
              </div>
            ) : <p className="text-xs text-slate-400">연결된 기술이 없습니다.</p>}
          </section>

          <section>
            <h4 className="mb-2 flex items-center gap-1.5 text-xs font-black uppercase tracking-wider text-slate-400"><BookOpen className="h-3.5 w-3.5" /> 관련 Study</h4>
          ...
          <section className="rounded-xl bg-slate-50 p-3 text-xs text-slate-500">
            <p><span className="font-bold text-slate-700">공개 상태</span> {competency.visible ? '공개' : '숨김'}</p>
            <p className="mt-1"><span className="font-bold text-slate-700">정렬 순서</span> {competency.displayOrder}</p>
            <p className="mt-1"><span className="font-bold text-slate-700">실무 근거</span> {competency.evidences.length}개</p>
          </section>
        </aside>
      </div>
    </article>
