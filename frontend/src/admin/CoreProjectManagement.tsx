...
import {
  ...
  type Experience,
  type ExperiencePlacementRequest,
} from '../lib/api';
...
export function CoreProjectManagement({ onCreateProject }: CoreProjectManagementProps) {
  ...
  const toDraft = () => placements.map((placement, index) => ({
  ...
  const setDetailSelected = (experienceId: number, detailId: number, selected: boolean) => {
    setDraft((current) => current.map((item) => {
      ...
      const detailIds = selected
        ? [...item.detailIds, detailId]
        : item.detailIds.filter((id) => id !== detailId);
      return { ...item, detailIds };
    }));
  };

  const selectAllDetails = (experience: Experience, selected: boolean) => {
    setDraft((current) => current.map((item) => item.experienceId === experience.id
      ? { ...item, detailIds: selected ? experience.details.map((detail) => detail.id) : [] }
      : item));
  };
    ...
    <div className="space-y-6">
      ...
      {saveMutation.error && (
        <p className="rounded-xl bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">
          {saveMutation.error instanceof Error ? saveMutation.error.message : '저장 중 오류가 발생했습니다.'}
        </p>
      )}
          ...
          <div className="divide-y divide-slate-100">
            {draft.map((placement, index) => {
              ...
              const displayedDetails = isEditing
                ? experience.details
                : experience.details.filter((detail) => placement.detailIds.includes(detail.id));
              return (
                <article
                  ...
                  className={`px-1 py-3 transition first:pt-0 last:pb-0 ${experience.details.length > 0 ? 'cursor-pointer' : ''} ${
                    draggedIndex === index ? 'bg-blue-50/70 ring-1 ring-blue-200' : 'bg-white'
                  }`}
                >
                    ...
                    {isEditing && (
                      <>
                        <label onClick={(event) => event.stopPropagation()} className="flex cursor-pointer items-center gap-1.5 text-xs font-bold text-slate-500">
                          <input
                            ...
                            onChange={(event) => setDraft((current) => current.map((item) =>
                              item.experienceId === placement.experienceId
                        ...
                        <div onClick={(event) => event.stopPropagation()} className="flex items-center gap-1">
                          <button type="button" onClick={() => move(index, -1)} disabled={index === 0} title="위로 이동" className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 disabled:opacity-25"><ArrowUp className="h-4 w-4" /></button>
                          <button type="button" onClick={() => move(index, 1)} disabled={index === draft.length - 1} title="아래로 이동" className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 disabled:opacity-25"><ArrowDown className="h-4 w-4" /></button>
                          <button type="button" onClick={() => remove(placement.experienceId)} title="편성에서 제거" className="rounded-lg p-2 text-slate-400 hover:bg-red-50 hover:text-red-600"><Trash2 className="h-4 w-4" /></button>
                        </div>
                      </>
                    )}
                  </div>
                        ...
                        <div className={`${isEditing ? 'ml-9' : 'ml-0'} border-t border-slate-200 pl-9 pt-3`}>
                          {isEditing && (
                            <div className="mb-2 flex items-center justify-between gap-3">
                              ...
                              <div onClick={(event) => event.stopPropagation()} className="flex items-center gap-1">
                                <button type="button" onClick={() => selectAllDetails(experience, true)} className="px-2 py-1 text-[11px] font-bold text-slate-500 hover:text-slate-950">전체 선택</button>
                                <button type="button" onClick={() => selectAllDetails(experience, false)} className="px-2 py-1 text-[11px] font-bold text-slate-400 hover:text-slate-700">전체 해제</button>
                              </div>
                            </div>
                          )}
                                ...
                                return (
                                  <div key={detail.id}>
                                    ...
                                    >
                                      ...
                                      {hasDetailContent && (
                                        <ChevronDown className={`mt-0.5 h-4 w-4 shrink-0 text-slate-400 transition-transform duration-300 ${isDetailExpanded ? 'rotate-180 text-slate-700' : ''}`} />
                                      )}
                                    </button>
                                            ...
                                            {detail.skills.length > 0 && (
                                              <div className="flex flex-wrap gap-1 pt-0.5">
                                                {detail.skills.map((skill) => (
                                                  <span key={skill.id} className="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-bold text-slate-600">{skill.name}</span>
                                                ))}
                                              </div>
                                            )}
                                          </div>
        ...
        {selectableExperiences.length === 0 ? (
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 py-4">
            ...
            <button
              type="button"
              onClick={onCreateProject}
              className="inline-flex items-center gap-1.5 rounded-lg bg-slate-900 px-3 py-2 text-xs font-bold text-white transition hover:bg-slate-800"
            >
