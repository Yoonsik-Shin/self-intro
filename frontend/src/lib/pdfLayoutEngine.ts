...
export function partitionAtomsIntoPages(
  ...
  for (let i = 0; i < atoms.length; i++) {
    ...
    const customGap = sectionGaps[atom.id] ?? (atom.isHeader ? sectionGaps[atom.sectionId] : undefined) ?? 0;
    ...
    if (!isForcedCurrentPage && atom.isHeader && i + 1 < atoms.length) {
      ...
      const nextCustomGap = sectionGaps[nextAtom.id] ?? (nextAtom.isHeader ? sectionGaps[nextAtom.sectionId] : undefined) ?? 0;
      const nextGap = nextCustomGap !== 0 ? nextCustomGap : 6;
      
      if (currentHeight + itemTotalHeight + nextHeight + nextGap > maxContentHeight && currentPageItems.length > 0) {
        pushHeaderToNextPage = true;
      }
    }

    if (!isForcedCurrentPage && ((currentHeight + itemTotalHeight > maxContentHeight && currentPageItems.length > 0) || pushHeaderToNextPage)) {
      startNewPage();
    }
