...
export type CodeLanguageChange = (position: { start: number; end: number }, language: string) => void;
...
function normalizeLanguage(language?: string) {
  ...
  return codeLanguages.find(({ value, aliases }) => value === language || aliases.some((alias) => alias === language))?.value ?? language;
}
  ...
  useEffect(() => {
    ...
    const render = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setError(renderError instanceof Error ? renderError.message : 'Mermaid 문법을 확인해 주세요.');
          setRendering(false);
        }
      }
    };
  ...
  if (!normalizedSource) {
    return (
      <div className="my-4 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5 text-center text-xs text-slate-500">
        ...
        <code className="mt-2 block font-mono text-slate-400">graph TD&nbsp;&nbsp;A[시작] --&gt; B[완료]</code>
      </div>
    );
  }
    ...
    <div className="my-4">
      {error && (
        <div className="mb-3 rounded-xl border border-red-200 bg-red-50 p-4">
          ...
          <pre className="mt-3 overflow-x-auto rounded-lg bg-slate-950 p-3 text-xs leading-5 text-slate-100">{source}</pre>
        </div>
      )}
      <div className={`relative min-h-24 overflow-x-auto rounded-xl border border-slate-200 bg-white p-4 ${error ? 'hidden' : ''}`}>
        {rendering && (
          <div className="absolute inset-0 z-10 grid place-items-center bg-white/80 text-xs font-semibold text-slate-400">
            다이어그램 렌더링 중...
          </div>
        )}
        <div ref={diagramRef} className="[&_svg]:mx-auto [&_svg]:h-auto [&_svg]:max-w-full" />
      </div>
    </div>
  );
}
...
export function MarkdownCode({ children, className, node, onLanguageChange }: CodeProps) {
  ...
  const selectedLanguage = codeLanguages.some(({ value }) => value === language) ? language : 'text';
  const selectedLabel = codeLanguages.find(({ value }) => value === selectedLanguage)?.label ?? 'Plain Text';
  ...
  return (
    <div className="my-4 overflow-hidden rounded-xl border border-slate-700 bg-slate-950 shadow-sm">
      <div className="flex h-10 items-center justify-between border-b border-slate-700 bg-slate-900 px-3">
        <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Code</span>
        {canChangeLanguage ? (
          <label className="relative">
            <span className="sr-only">코드 언어</span>
            <select
              value={selectedLanguage}
              onChange={(event) => onLanguageChange({ start, end }, event.target.value)}
          ...
          <span className="rounded-md border border-slate-700 bg-slate-800 px-2.5 py-1 text-xs font-semibold text-slate-300">{selectedLabel}</span>
        )}
      </div>
      <SyntaxHighlighter
